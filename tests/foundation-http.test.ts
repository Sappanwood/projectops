...
function setup() {
  ...
  const cli = (...args: string[]) => {
    ...
    assert.equal(runCli(args, { stdout: s => stdout.push(s), stderr: s => stderr.push(s) }, workspaceDir), 0, stderr.join("\n"));
  ...
  const { item } = cli("backlog", "add", "repo", "-T", "Original", "-c", "feature", "--priority", "P1", "-b", "Original requirements", "--json");
...
test("HTTP task content editing preserves stale input and rejects mixed mutations", async () => {
  ...
  const patch = (body: unknown, origin = server.origin) => fetch(url, {
    method: "PATCH", headers: { "content-type": "application/json", origin }, body: JSON.stringify(body),
  });
  try {
    const updated = await patch({ title: "Revised", body: "New acceptance criteria", expected_revision: item.revision });
    ...
    assert.equal((await patch({ title: "Mixed", status: "done", expected_revision: item.revision })).status, 400);
    assert.equal((await patch({ title: "Wrong origin", expected_revision: item.revision }, "https://example.com")).status, 403);
...
test("HTTP execution control survives reconnect, blocks duplicate starts and confirms interruption", async () => {
  ...
  const runner = { start() { starts++; return {
    completion: new Promise<RunnerResult>(resolve => { finish = resolve; }), stop() { stops++; },
  }; } };
  ...
  const post = (suffix: string, body: unknown) => fetch(`${endpoint()}${suffix}`, {
    method: "POST", headers: { "content-type": "application/json", origin: server.origin }, body: JSON.stringify(body),
  });
  try {
    const input = { item_id: item.id, expected_revision: item.revision, instructions: "Controlled test" };
    ...
    assert.equal((await post(`/${unknown.id}/confirm-interrupted`, { expected_revision: unknown.revision, note: "Runner inspected and stopped" })).status, 200);
...
test("HTTP Plan revision previews without writing and requires exact confirmation", async () => {
  ...
  const draft = { title: "Revision HTTP", goal: "Before", items: [{ key: "work", title: "Work", item_type: "task", priority: "P1", body: "Do work", depends_on: [] }] };
  ...
  const post = (body: unknown) => fetch(`${url}/revision`, {
    method: "POST", headers: { "content-type": "application/json", origin: server.origin }, body: JSON.stringify(body),
  });
  try {
    ...
    const applied = await post({ draft: changed, expected_revision: before.revision, confirm: receipt.confirmation_token });
    assert.equal(applied.status, 200);
    assert.equal((await applied.json()).data.applied, true);
    assert.equal((await post({ draft: changed, expected_revision: before.revision, confirm: receipt.confirmation_token })).status, 409);
    assert.equal((await post({ draft: changed, expected_revision: before.revision, workspace: "/tmp/other" })).status, 400);
  } finally { await server.close(); rmSync(workspaceDir, { recursive: true, force: true }); }
});
