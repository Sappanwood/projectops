...
test("built execution CLI records external work, rejects stale input and requires evidence before acceptance", () => {
  ...
  const call = (args: string[], expected = 0) => {
    const p = spawnSync(process.execPath, [cliPath, ...args, "--json"], { cwd: workspace, encoding: "utf8" });
  ...
  try {
    ...
    const { item } = call(["backlog", "add", "repo", "-T", "CLI work", "-c", "feature", "--priority", "P1"]);
    const stale = call(["execution", "create", "repo", item.id, "--expected-revision", "stale"], 1);
    assert.equal(stale.error.code, "REVISION_MISMATCH");
    let a = call(["execution", "create", "repo", item.id, "--expected-revision", item.revision, "--instructions", "Explicit fixture input"]).data.attempt;
    ...
    a = call(["execution", "finish", "repo", a.id, "--expected-revision", a.revision, "--outcome", "succeeded", "--summary", "Fixture finished"]).data.attempt;
    assert.equal(call(["execution", "accept", "repo", a.id, "--expected-revision", a.revision, "--note", "No evidence"], 1).ok, false);
    writeFileSync(path.join(workspace, "evidence.txt"), "Fixture assertions passed.");
    a = call(["execution", "verify", "repo", a.id, "--expected-revision", a.revision, "--command", "fixture check", "--outcome", "passed", "--evidence-file", "evidence.txt"]).data.attempt;
    a = call(["execution", "accept", "repo", a.id, "--expected-revision", a.revision, "--note", "Fixture accepted"]).data.attempt;
