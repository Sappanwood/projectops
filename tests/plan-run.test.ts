...
import {
  createPlanRun,
  showPlanRun,
  validatePlanRunCompletion,
  PlanRunRuntime,
} from "../src/application/planRunApi.js";
...
function setup(chain = true) {
  ...
  const cli = (args: string[]) => {
    const output: string[] = [];
    assert.equal(
      runCli(args, { stdout: (x) => output.push(x), stderr: (x) => output.push(x) }, workspaceDir),
      0,
      output.join("\n"),
    );
  };
  ...
  writeFileSync(
    ...
    JSON.stringify({
      ...
      id: planId,
      title: "Chain",
      goal: "Deliver both tasks",
      status: "approved",
      approval: { approved_at: new Date().toISOString(), review_note: "Fixture approval" },
      items: ["first", "second"].map((key, i) => ({
        key,
        title: key,
        item_type: "task",
        priority: "P1",
        body: `Implement ${key}`,
        depends_on: chain && i ? ["first"] : [],
      })),
    }),
  );
  ...
  const execution = new ExecutionRuntime({
    start() {
      return {
        completion: new Promise<RunnerResult>((resolve) => controls.push(resolve)),
        stop() {},
      };
    },
  });
  const scheduler = new PlanRunRuntime(execution);
  const create = (instructions?: string) =>
    data(
      createPlanRun({
        ...q,
  ...
  const accept = (run: PlanRun) => {
    const attemptId = run.nodes
      .find((n) => n.attempt_ids.length && n.state !== "accepted")!
      .attempt_ids.at(-1)!;
    ...
    attempt = data(
      verifyExecution({
        ...
        outcome: "passed",
        evidence: "fixture assertions passed",
      }),
    ).attempt;
    return data(
      decideExecution({
...
test("serial run persists its model for successors after reload", async () => {
  ...
  let run = data(
    createPlanRun({ ...f.q, expectedRevision: computePlanRevision(f.plan), model }),
  ).run;
  model.id = "two";
...
test("serial plan run freezes inputs and dispatches successor only after verification and acceptance", async () => {
  ...
  assert.equal(
    run.nodes.every((n) => n.state === "accepted"),
    true,
  );
...
test("failure pauses dispatch; explicit resume retains history and starts a retry", async () => {
  ...
  assert.equal(f.controls.length, 1);
  run = data(
    f.scheduler.resume({ ...f.mutation(run), note: "Failure inspected; retry approved" }),
  ).run;
  run = data(f.scheduler.advance(f.mutation(run))).run;
...
test("restart uncertainty pauses plan and prevents redispatch or resume", () => {
  ...
  assert.equal(run.state, "paused");
  assert.equal(
    f.scheduler.resume({ ...f.mutation(run), note: "Resume without confirming" }).ok,
    false,
  );
  assert.equal(f.controls.length, 1);
});
      ...
      reuse: [
        {
          itemId: q.itemId,
          attemptId: attempt.id,
          note: "Confirmed applicable to this run baseline",
        },
      ],
    }),
  ).run;
...
test("stopping an obsolete scope preserves its history and allows creating a new snapshot", () => {
  ...
  assert.equal(run.state, "paused");
  assert.equal(
    f.scheduler.resume({ ...f.mutation(run), note: "Old scope cannot resume" }).ok,
    false,
  );
  assert.equal(f.scheduler.closeStopped({ ...f.mutation(run), note: "" }).ok, false);
  run = data(
    f.scheduler.closeStopped({ ...f.mutation(run), note: "Superseded by approved revision" }),
  ).run;
...
test("scope termination refuses active or unknown work", () => {
  ...
  assert.equal(
    f.scheduler.closeStopped({ ...f.mutation(run), note: "Cannot abandon running process" }).ok,
    false,
  );
  new ExecutionRuntime().recover(f.q);
  assert.equal(
    f.scheduler.closeStopped({ ...f.mutation(run), note: "Unknown also blocks" }).ok,
    false,
  );
});
...
test("resume cannot convert accepted but drifted output into a valid predecessor", async () => {
  ...
  assert.equal(
    f.scheduler.resume({ ...f.mutation(run), note: "Inspected changed code", baselineDigest }).ok,
    false,
  );
  assert.equal(f.controls.length, 1);
});
...
test("input drift pauses dispatch while ended attempt state remains observable", async () => {
  ...
  assert.equal(f.controls.length, 1);
  assert.equal(
    f.scheduler.closeStopped({ ...f.mutation(run), note: "Stopped obsolete scope" }).ok,
    true,
  );
});
...
test("completion validation reads current inputs, final baseline and durable evidence without writing", async () => {
  ...
  writeFileSync(path.join(f.repo, "code.txt"), "first and second accepted output");
  writeFileSync(
    path.join(f.q.workspaceDir, "ops/repo/executions", last.verifications[0]!.evidence_ref),
    "replaced evidence",
  );
  assert.equal(validatePlanRunCompletion(query).ok, false);
  assert.equal(readFileSync(file, "utf8"), before);
...
test("run freezes additional instructions and preserves them in initial and retried attempts", async () => {
  ...
  run = data(f.scheduler.advance(f.mutation(run))).run;
  const first = data(
    showExecution({ ...f.q, attemptId: run.nodes[0]!.attempt_ids.at(-1)! }),
  ).attempt;
  ...
  run = data(f.scheduler.advance(f.mutation(run))).run;
  run = data(
    f.scheduler.resume({ ...f.mutation(run), note: "Retry the same read-only review" }),
  ).run;
  run = data(f.scheduler.advance(f.mutation(run))).run;
  const retried = data(
    showExecution({ ...f.q, attemptId: run.nodes[0]!.attempt_ids.at(-1)! }),
...
test("historical Plan completion permits later development but still checks accepted evidence", async () => {
  ...
  const evidence = path.join(
    f.q.workspaceDir,
    "ops/repo/executions",
    accepted.verifications[0]!.evidence_ref,
  );
  ...
  assert.equal(
    readPlan(path.join(f.q.workspaceDir, "ops/repo/plans"), f.plan.id).status,
    "approved",
  );
