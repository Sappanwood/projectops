...
import {
  createParallelRun,
  showParallelRun,
  validateParallelRunCompletion,
  ParallelRunRuntime,
} from "../src/application/parallelRunApi.js";
...
function setup(options: { permit?: boolean; shared?: boolean; serialNode?: boolean } = {}) {
  const workspaceDir = mkdtempSync(path.join(tmpdir(), "parallel-run-"));
  const repo = path.join(workspaceDir, "repo");
  const cli = (args: string[]) => {
    const out: string[] = [];
    assert.equal(
      runCli(args, { stdout: (x) => out.push(x), stderr: (x) => out.push(x) }, workspaceDir),
      0,
      out.join("\n"),
    );
  };
  ...
  git(repo, "init", "-q");
  git(repo, "config", "user.name", "Parallel Fixture");
  ...
  writeFileSync(
    path.join(workspaceDir, "ops/repo/plans", `${planId}.json`),
    JSON.stringify({
      schema: "plan/Plan@1",
      id: planId,
      title: "Diamond",
      goal: "Deliver diamond",
      status: "approved",
      ...
      items: ["a", "b", "c", "d"].map((key) => ({
        ...
        priority: "P1",
        body: `Implement ${key}`,
        parallel: !(options.serialNode && key === "c"),
        resources: options.shared && ["b", "c"].includes(key) ? ["database"] : [],
    ...
    start(a, context) {
      return {
        completion: new Promise((resolve) =>
          running.set(a.item_id, { dir: context.repo, end: resolve }),
        ),
        stop() {},
      };
    },
  ...
  const finish = async (
    run: ParallelRun,
    key: string,
    outcome: RunnerResult["outcome"] = "succeeded",
  ) => {
    const node = run.nodes.find((n) => n.key === key)!;
    const handle = running.get(node.item_id)!;
    if (outcome === "succeeded") {
      writeFileSync(path.join(handle.dir, `${key}.txt`), `${key}\n`);
      git(handle.dir, "add", ".");
      git(handle.dir, "commit", "-qm", `Implement ${key}`);
    }
    handle.end({ outcome, summary: outcome });
    await new Promise((resolve) => setImmediate(resolve));
    if (outcome === "succeeded") {
      const attemptId = node.attempt_ids.at(-1)!;
      let a = data(showExecution({ ...q, attemptId })).attempt;
      a = data(
        verifyExecution({
          ...q,
          attemptId,
          ...
          outcome: "passed",
          evidence: "Passed in owned checkout",
        }),
      ).attempt;
      data(
        decideExecution({
          ...
          expectedRevision: a.revision,
          decision: "accepted",
          note: "Reviewed owned checkout",
        }),
      );
    }
  };
  const land = async (run: ParallelRun, key: string) =>
    data(await scheduler.land({ ...mutation(run), nodeKey: key })).run;
  return {
    q,
    repo,
    baseCommit,
...
test("parallel diamond executes two real handles in owned checkouts and only landed dependencies unlock join", async () => {
  ...
  assert.notEqual(f.running.values().next().value!.dir, f.repo);
  await f.finish(run, "a");
  run = data(f.scheduler.advance(f.mutation(run))).run;
  ...
  const b = run.nodes.find((n) => n.key === "b")!;
  const c = run.nodes.find((n) => n.key === "c")!;
  assert.notEqual(f.running.get(b.item_id)!.dir, f.running.get(c.item_id)!.dir);
  for (const node of [b, c])
    assert.equal(readFileSync(path.join(f.running.get(node.item_id)!.dir, "a.txt"), "utf8"), "a\n");
  await f.finish(run, "b");
  await f.finish(run, "c");
  run = data(f.scheduler.advance(f.mutation(run))).run;
  run = await f.land(run, "b");
  assert.equal(run.nodes.find((n) => n.key === "d")!.state, "pending");
  run = await f.land(run, "c");
  run = data(f.scheduler.advance(f.mutation(run))).run;
  const d = run.nodes.find((n) => n.key === "d")!;
  for (const file of ["b.txt", "c.txt"])
    assert.equal(
      readFileSync(path.join(f.running.get(d.item_id)!.dir, file), "utf8").trim(),
      file[0],
    );
  await f.finish(run, "d");
  run = data(f.scheduler.advance(f.mutation(run))).run;
  run = await f.land(run, "d");
...
test("parallel execution requires explicit permission and respects resources and serial nodes", async () => {
  const noPermit = setup({ permit: false });
  assert.equal(createParallelRun(noPermit.createRequest).ok, false);
...
test("failed branch pauses dispatch, keeps its sibling and blocks join until explicit resume", async () => {
  ...
  run = await f.land(run, "a");
  run = data(f.scheduler.advance(f.mutation(run))).run;
  await f.finish(run, "b", "failed");
  run = data(f.scheduler.advance(f.mutation(run))).run;
  assert.equal(run.state, "paused");
  assert.equal(run.nodes.find((n) => n.key === "c")!.state, "running");
  assert.equal(run.nodes.find((n) => n.key === "d")!.state, "pending");
  run = data(
    f.scheduler.resume({ ...f.mutation(run), note: "Inspect failure and retry only B" }),
  ).run;
  run = data(f.scheduler.advance(f.mutation(run))).run;
  ...
  restarted.recover(f.q);
  run = data(f.scheduler.advance(f.mutation(run))).run;
  assert.equal(run.state, "paused");
  assert.equal(f.scheduler.resume({ ...f.mutation(run), note: "Cannot ignore unknown" }).ok, false);
});
test("landing validation failure and integration ref drift never unlock descendants", async () => {
  const f = setup();
  f.createRequest.commands = [[process.execPath, "-e", "process.exit(1)"]];
  let run = data(f.scheduler.advance(f.mutation(f.create()))).run;
  await f.finish(run, "a");
  run = data(f.scheduler.advance(f.mutation(run))).run;
  run = await f.land(run, "a");
  ...
  const drift = setup();
  let changed = data(drift.scheduler.advance(drift.mutation(drift.create()))).run;
  await drift.finish(changed, "a");
  changed = data(drift.scheduler.advance(drift.mutation(changed))).run;
  const a = changed.nodes[0]!;
  const nodeHead = git(drift.running.get(a.item_id)!.dir, "rev-parse", "HEAD");
  git(drift.repo, "update-ref", changed.workspace.integrationRef, nodeHead, drift.baseCommit);
  assert.equal(
    (await drift.scheduler.land({ ...drift.mutation(changed), nodeKey: "a" })).ok,
    false,
  );
...
test("accepted landing failure can explicitly request rework without rewriting historical acceptance", async () => {
  const f = setup();
  f.createRequest.commands = [
    [process.execPath, "-e", "if (!require('node:fs').existsSync('repair.txt')) process.exit(1)"],
  ];
  let run = data(f.scheduler.advance(f.mutation(f.create()))).run;
  await f.finish(run, "a");
  run = data(f.scheduler.advance(f.mutation(run))).run;
  ...
  run = data(
    f.scheduler.rework({
      ...f.mutation(run),
      nodeKey: "a",
      note: "Integration check requires repair file",
    }),
  ).run;
  assert.equal(
    data(showBacklogItem({ ...f.q, itemId: run.nodes[0]!.item_id })).item.status,
    "in_progress",
  );
  ...
  run = data(
    f.scheduler.resume({
      ...f.mutation(run),
      note: "Retry accepted work after integration rejection",
    }),
  ).run;
  run = data(f.scheduler.advance(f.mutation(run))).run;
  ...
  assert.equal(
    f.scheduler.rework({ ...f.mutation(run), nodeKey: "a", note: "Cannot reopen landed work" }).ok,
    false,
  );
});
...
test("managed acceptance requires committed clean submission and verification of that commit", async () => {
  ...
  const node = run.nodes[0]!;
  const handle = f.running.get(node.item_id)!;
  writeFileSync(path.join(handle.dir, "uncommitted.txt"), "not submitted");
  handle.end({ outcome: "succeeded", summary: "Uncommitted work" });
  await new Promise((resolve) => setImmediate(resolve));
  const attemptId = node.attempt_ids.at(-1)!;
  ...
  attempt = data(
    verifyExecution({
      ...
      command: "check",
      outcome: "passed",
      evidence: "Checks passed on uncommitted files",
    }),
  ).attempt;
  const rejected = decideExecution({
    ...f.q,
    attemptId,
    expectedRevision: attempt.revision,
    decision: "accepted",
    note: "Try premature acceptance",
  });
  assert.equal(rejected.ok, false);
  if (!rejected.ok) assert.match(rejected.error.message, /commit|submit/i);
  git(handle.dir, "add", ".");
  git(handle.dir, "commit", "-qm", "Submit work");
  assert.equal(
    decideExecution({
      ...f.q,
      attemptId,
      expectedRevision: attempt.revision,
      decision: "accepted",
      note: "Old verification is stale",
    }).ok,
    false,
  );
  attempt = data(
    verifyExecution({
      ...
      command: "check",
      outcome: "passed",
      evidence: "Submitted commit checked",
    }),
  ).attempt;
  assert.equal(
    decideExecution({
      ...
      expectedRevision: attempt.revision,
      decision: "accepted",
      note: "Reviewed submitted commit",
    }).ok,
    true,
  );
});
...
test("parallel lifecycle continues to observe ended work after input drift", async () => {
  ...
  assert.equal(
    f.scheduler.closeStopped({ ...f.mutation(run), note: "All work settled; close old input" }).ok,
    true,
  );
});
...
test("explicitly accepting integration ref drift aligns only its owned detached checkout", async () => {
  ...
  assert.equal(run.state, "paused");
  assert.equal(
    f.scheduler.resume({ ...f.mutation(run), note: "Missing inspected head" }).ok,
    false,
  );
  run = data(
    f.scheduler.resume({
      ...f.mutation(run),
      note: "Inspected ref includes accepted A",
      integrationHead: next,
    }),
  ).run;
...
test("a run started without a runner can resume after a runner is configured", () => {
  const f = setup();
  const unavailable = new ParallelRunRuntime(new ExecutionRuntime());
...
test("failed launch records its prepared worktree and safely reuses it on resume", () => {
  ...
  f.execution.startManaged = () =>
    applicationFailure("EXECUTION_CONFLICT", "Fixture launch rejected before creating an attempt");
  let run = data(f.scheduler.advance(f.mutation(f.create()))).run;
  assert.equal(run.state, "paused");
  ...
  run = data(
    f.scheduler.resume({ ...f.mutation(run), note: "Transient start failure resolved" }),
  ).run;
  run = data(f.scheduler.advance(f.mutation(run))).run;
  assert.equal(run.nodes[0]!.state, "running");
...
test("landing rechecks selected acceptance after checkout or evidence changes", async () => {
  for (const changed of ["checkout", "evidence"]) {
    ...
    if (changed === "checkout") {
      ...
      git(node.workspace!.dir, "add", ".");
      git(node.workspace!.dir, "commit", "-qm", "Unreviewed addition");
    } else {
      const attempt = data(showExecution({ ...f.q, attemptId: node.attempt_ids.at(-1)! })).attempt;
      writeFileSync(
        path.join(f.q.workspaceDir, "ops/repo/executions", attempt.verifications[0]!.evidence_ref),
        "changed evidence",
      );
    }
...
test("a failure after integration ref publication remains awaiting inspection and cannot replay", async () => {
  const f = setup();
  let run = f.create();
  const indexLock = path.join(
    git(run.workspace.integrationDir, "rev-parse", "--absolute-git-dir"),
    "index.lock",
  );
  run.commands = [
    [process.execPath, "-e", `require('node:fs').writeFileSync(${JSON.stringify(indexLock)}, '')`],
  ];
