...
test("Backlog ignores a pending mutation response after switching projects and prevents double submit", async () => {
  ...
  const api = {
    ...
    updateBacklog: async () => {
      mutations++;
      return new Promise<ApiResult<BacklogMutationReceipt>>((resolve) => {
        finish = resolve;
      });
    },
  };
  ...
  assert.match(renderBacklogPanel(controller.getState()), /依赖：ALP-002/);
  ...
  finish({
    ...
    data: {
      ...
      revision: "new",
      before: item,
      result: { ...item, status: "done", revision: "new" },
    },
  });
  await update;
...
test("Backlog exposes list diagnostics and sorts complete items within each status", async () => {
  ...
  const items = ["ALP-003", "ALP-001", "ALP-002"].map((id) =>
    parseItemFile(`---\nid: ${id}\ntitle: ${id}\n---\nBody`),
  );
  const api = {
    ...createApiClient(),
