...
function materializePlan(workspace: string): void {
  ...
  writeFileSync(draft, `${JSON.stringify({
    ...
    items: [
      { key: "prepare", title: "Prepare release", item_type: "task", priority: "P1", body: "Prepare." },
      { key: "publish", title: "Publish release", item_type: "task", priority: "P1", body: "Publish.", depends_on: ["prepare"] },
    ],
  }, null, 2)}\n`, "utf8");
...
function materializePendingPlan(workspace: string): void {
  ...
  writeFileSync(draft, `${JSON.stringify({
    ...
    items: [
      { key: "audit", title: "Audit service", item_type: "task", priority: "P1", body: "Audit." },
    ],
  }, null, 2)}\n`, "utf8");
  assert.equal(run(["plan", "create", "repo-a", "--input", draft], workspace).code, 0);
  assert.equal(run(["plan", "approve", "repo-a", "plan-maintenance-workflow", "--review-note", "Reviewed."], workspace).code, 0);
  assert.equal(run(["plan", "materialize", "repo-a", "plan-maintenance-workflow"], workspace).code, 0);
}
...
test("report create/list/show expose a completed Report and preserve no-clobber", () => {
  ...
  try {
    ...
    const created = run([
      "report", "create", "repo-a", "plan-release-workflow",
    ...
    assert.deepEqual(JSON.parse(created.stdout[0] ?? "null"), {
      ...
      report: {
        ...
        title: "Release workflow",
        project: "repo-a",
        created_at: (JSON.parse(created.stdout[0] ?? "null") as { report: { created_at: string } }).report.created_at,
        outcome: "completed",
        plan: "project-ops:plans/plan-release-workflow.json",
        backlog: [
          { id: "REP-001", status: "done", revision: (JSON.parse(created.stdout[0] ?? "null") as { report: { backlog: [{ revision: string }] } }).report.backlog[0].revision, uri: "project-ops:backlog/items/REP-001.md" },
          { id: "REP-002", status: "done", revision: (JSON.parse(created.stdout[0] ?? "null") as { report: { backlog: [{ revision: string }, { revision: string }] } }).report.backlog[1].revision, uri: "project-ops:backlog/items/REP-002.md" },
        ],
    ...
    const duplicate = run([
      "report", "create", "repo-a", "plan-release-workflow",
      "--report-id", "report-release-zeta", "--verification", "npm test", "--json",
    ], workspace);
    assert.equal(duplicate.code, 1);
    assert.deepEqual(JSON.parse(duplicate.stdout[0] ?? "null"), { ok: false, error: "Report already exists: report-release-zeta" });
    assert.equal(duplicate.stderr.length, 0);

    const second = run([
      "report", "create", "repo-a", "plan-release-workflow",
      "--report-id", "report-release-alpha", "--verification", "npm test", "--json",
    ], workspace);
      ...
      reports: [
        {
          ...
          title: "Release workflow",
          project: "repo-a",
          created_at: (JSON.parse(second.stdout[0] ?? "null") as { report: { created_at: string } }).report.created_at,
          outcome: "completed",
          plan: "project-ops:plans/plan-release-workflow.json",
        },
        {
    ...
    assert.equal(shown.code, 0, shown.stderr.join("\n"));
    assert.equal((JSON.parse(shown.stdout[0] ?? "null") as { id: string; outcome: string }).id, "report-release-alpha");
    assert.equal(existsSync(path.join(workspace, "ops", "repo-a", "reports", "report-release-alpha.md")), true);
  } finally {
...
test("report create requires verification and explicit partial acceptance", () => {
  ...
  try {
    ...
    const noEvidence = run(["report", "create", "repo-a", "plan-release-workflow", "--json"], workspace);
    assert.equal(noEvidence.code, 1);
    assert.deepEqual(JSON.parse(noEvidence.stdout[0] ?? "null"), { ok: false, error: "--verification is required at least once" });

    const unfinished = run([
      "report", "create", "repo-a", "plan-release-workflow", "--verification", "npm test", "--json",
    ], workspace);
    assert.equal(unfinished.code, 1);
    assert.match(JSON.parse(unfinished.stdout[0] ?? "null").error, /partial acceptance/i);
    assert.equal(existsSync(path.join(workspace, "ops", "repo-a", "reports", "report-release-workflow.md")), false);

    const partial = run([
      "report", "create", "repo-a", "plan-release-workflow",
      "--verification", "npm test",
      "--partial-acceptance", "Publish waits for the registry window.",
      "--json",
    ], workspace);
    assert.equal(partial.code, 0, partial.stderr.join("\n"));
    const report = JSON.parse(partial.stdout[0] ?? "null").report as { outcome: string; deviations: string[] };
...
test("built CLI report create returns completed, partial, no-clobber, and invalid-reference results", () => {
  ...
  try {
    ...
    const completed = runBuilt([
      "report", "create", "repo-a", "plan-release-workflow",
      "--report-id", "report-release-built",
      "--verification", "npm test",
      "--json",
    ], workspace);
    assert.equal(completed.code, 0, completed.stderr);
    assert.equal(completed.stderr, "");
    const completedJson = JSON.parse(completed.stdout) as { ok: boolean; report?: { outcome: string } };
    assert.equal(completedJson.ok, true);
    assert.equal(completedJson.report?.outcome, "completed");

    const partial = runBuilt([
      "report", "create", "repo-a", "plan-maintenance-workflow",
      "--report-id", "report-maintenance-built",
      "--verification", "npm test",
      "--partial-acceptance", "Audit waits for the maintenance window.",
      "--json",
    ], workspace);
    ...
    const duplicateJson = JSON.parse(duplicate.stdout) as { ok: boolean; report?: unknown; error?: string };
    ...
    const invalidReference = runBuilt([
      "report", "create", "repo-a", "plan-does-not-exist",
      "--verification", "npm test",
      "--json",
    ], workspace);
    assert.equal(invalidReference.code, 1);
    assert.equal(invalidReference.stderr, "");
    const invalidReferenceJson = JSON.parse(invalidReference.stdout) as { ok: boolean; report?: unknown; error?: string };
