...
function setup() {
  ...
  function run(args: string[]) {
    const errors: string[] = [];
    assert.equal(
      runCli(args, { stdout() {}, stderr: (message) => errors.push(message) }, root),
      0,
      errors.join("\n"),
    );
  }
...
function snapshot(root: string): Record<string, string> {
  ...
  for (const entry of readdirSync(root, { withFileTypes: true })) {
    if (entry.isDirectory()) {
      for (const [name, body] of Object.entries(snapshot(path.join(root, entry.name))))
...
test("four read-only pages expose full typed details through HTTP without writes", async () => {
  ...
  try {
    ...
    writeFileSync(
      path.join(ops, "plans/plan-delivery.json"),
      JSON.stringify({
        schema: "plan/Plan@1",
        id: "plan-delivery",
        title: "Delivery",
        ...
        materialization: {
          ...
          mapping: { first: "ALP-001", second: "ALP-002" },
        },
        items: [
          {
            key: "first",
    ...
    const report: Report = {
      ...
      outcome: "partial",
      plan: "project-ops:plans/plan-delivery.json",
      backlog: [{ id: "ALP-001", status: "done", uri: "project-ops:backlog/items/ALP-001.md" }],
      verification: ["npm test passed"],
      deviations: ["Accepted partial"],
      workarounds: ["Temporary fixture"],
      repo_docs: ["README.md"],
    ...
    for (let i = 0; i < 8; i++) {
      ...
      writeFileSync(
        path.join(root, `retrospectives/${status}/record-${i}.md`),
        serializeRetrospective({
          schema: "retrospective/Retrospective@1",
          id: `record-${i}`,
          created_at: "2026-09-05",
          project: i === 7 ? null : "alpha",
          task: `ALP-00${i}`,
          status,
    ...
    for (const text of [
      ...
      assert.ok(plans.includes(text), text);
    ...
    for (const text of [
      ...
      assert.ok(reports.includes(text), text);
    assert.doesNotMatch(reports, /<script>/);
    const retro = renderReadPages("retrospectives", data, {
      project: "alpha",
      status: "active",
      task: "ALP-004",
    });
    ...
    assert.match(
      renderReadPages("retrospectives", data, { project: "", status: "", task: "" }),
      /record-7/,
    );
    ...
    for (const name of ["README.md", "AGENTS.md", "docs/PRODUCT_SPEC.md", "docs/ARCHITECTURE.md"])
      assert.ok(docs.includes(name));
    assert.deepEqual(snapshot(root), before);
    assert.equal(
      (await fetch(`${server.origin}/api/projects/alpha/read-pages`, { method: "POST" })).status,
      405,
    );
    assert.equal(
      (await fetch(`${server.origin}/api/projects/alpha/read-pages?path=README.md`)).status,
      400,
    );
...
test("each page presents empty data and domain diagnostics without hiding healthy records", async () => {
  ...
  try {
    ...
    assert.match(
      renderReadPages("docs", data, { project: "alpha", status: "", task: "" }),
      /missing/,
    );
    ...
    for (const source of ["plans", "reports", "retrospectives"])
      assert.ok(
        data.diagnostics.some(
          (diagnostic) => diagnostic.source === source && diagnostic.code === "ARTIFACT_INVALID",
        ),
      );
...
test("mounted Workbench loads read pages, submits filters and refreshes authority data", async () => {
  ...
  try {
    writeFileSync(
      path.join(root, "retrospectives/inbox/visible.md"),
      serializeRetrospective({
        schema: "retrospective/Retrospective@1",
        id: "visible",
        created_at: "2026-09-05",
    ...
    let route: import("../src/web/types.js").RouteState = {
      projectId: "alpha",
      view: "retrospectives",
    };
    ...
    handlers.submit!({
      target: {
        ...
        elements: { namedItem: (name: string) => ({ value: values[name] }) },
      },
      preventDefault() {},
    });
...
test("read page requests discard stale responses and expose retry after a read failure", async () => {
  const { createWorkbenchApp } = await import("../src/web/app.js");
  const { createApiClient } = await import("../src/web/apiClient.js");
  const { getWorkbenchProjectOverview, getWorkbenchWorkspaceOverview } = await import(
    "../src/application/workbenchReadModel.js"
  );
  ...
  const pages = (title: string): WorkbenchReadPages => ({
    plans: [
      {
  ...
  const app = createWorkbenchApp({
    ...
    apiClient: {
      ...
      getProjectOverview: async (projectId) =>
        getWorkbenchProjectOverview({ workspaceDir: root, projectId }),
...
test("Plan execution counts current mapped tasks, isolates unreadable targets and never writes", async () => {
  ...
  try {
    ...
    writeFileSync(
      path.join(ops, "plans/plan-progress.json"),
      JSON.stringify({
        schema: "plan/Plan@1",
        id: "plan-progress",
        title: "Progress",
        goal: "Current execution",
        status: "approved",
        approval: { approved_at: "2026-09-05", review_note: "Fixture" },
        materialization: {
          materialized_at: "2026-09-05",
          mapping: Object.fromEntries(keys.map((key, i) => [key, `ALP-00${i + 1}`])),
        },
        items: keys.map((key) => ({
          key,
          title: `Planned ${key}`,
          item_type: key === "epic" ? "epic" : "task",
          priority: "P1",
          body: "Scope",
          depends_on: [],
        })),
    ...
    assert.deepEqual(
      execution.items.map((item) => item.status),
      ["todo", "in_progress", "done", "unreadable", "done", "unreadable"],
    );
    ...
    assert.deepEqual(snapshot(root), before);
    writeFileSync(
      path.join(ops, "backlog/items/ALP-001.md"),
      readFileSync(path.join(ops, "backlog/items/ALP-001.md"), "utf8").replace(
        "status: todo",
        "status: done",
      ),
    );
...
test("unmaterialized and zero-task Plans have no misleading completion percentage", async () => {
  ...
  try {
    ...
    writeFileSync(
      path.join(ops, "plans/plan-draft.json"),
      JSON.stringify({ ...base, id: "plan-draft", status: "draft" }),
    );
    writeFileSync(
      path.join(ops, "plans/plan-empty.json"),
      JSON.stringify({
        ...base,
        id: "plan-empty",
        status: "approved",
        approval: { approved_at: "2026-09-05", review_note: "Fixture" },
        materialization: { materialized_at: "2026-09-05", mapping: {} },
      }),
    );
  ...
  try {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        goal: "Evidence",
        items: [{ key: "task", title: "Task", item_type: "task", priority: "P1", body: "Scope" }],
      }),
    );
    ...
    const base: Report = {
      ...
      outcome: "partial",
      plan: "project-ops:plans/plan-delivery-links.json",
