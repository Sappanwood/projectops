...
import {
  ...
  alignIntegration,
  type LandingUncertainError,
} from "../src/planRun/worktrees.js";
...
function setup() {
  ...
  git(repo, "config", "user.email", "plan-test@example.invalid");
...
test("diamond workspaces land serially over the latest integration without changing user checkout", async () => {
  const input = setup();
  const run = createRunWorkspace(input);
  const a = createNodeWorkspace(run, "a");
  const b = createNodeWorkspace(run, "b");
  commit(a.dir, "a.txt", "A\n");
  commit(b.dir, "b.txt", "B\n");
  const [first, second] = await Promise.all([
    landNode(run, a, passed),
    landNode(run, b, async (candidate) => {
  ...
  assert.equal(second.outcome, "landed");
...
test("workspace and ref creation reject no-clobber and static symlink violations", () => {
  ...
  const node = createNodeWorkspace(run, "node");
  assert.throws(() => createNodeWorkspace(run, "node"));
  assert.equal(git(node.dir, "rev-parse", "HEAD"), input.baseCommit);
  const unsafe = setup();
  const outside = mkdtempSync(path.join(tmpdir(), "plan-worktrees-outside-"));
  ...
  git(
    collision.repo,
    "update-ref",
    "refs/heads/pops/run-fixture/integration",
    collision.baseCommit,
  );
  assert.throws(() => createRunWorkspace(collision));
  assert.equal(
    git(collision.repo, "rev-parse", "refs/heads/pops/run-fixture/integration"),
    collision.baseCommit,
  );
...
test("merge conflicts, failed validation and integration ref drift preserve evidence without landing", async () => {
  ...
  const conflict = await landNode(run, b, async () => {
    throw Error("must not verify conflicts");
  });
  assert.equal(conflict.outcome, "conflict");
  assert(existsSync(conflict.evidenceFile));
  assert(existsSync(conflict.candidateDir));
  assert.equal(git(input.repo, "rev-parse", run.integrationRef), before);
  const c = createNodeWorkspace(run, "c");
  const nodeCommit = commit(c.dir, "c.txt", "C\n");
  const failed = await landNode(run, c, async () => ({
    passed: false,
    evidence: "Test rejected C.",
  }));
  assert.equal(failed.outcome, "verification_failed");
  assert.match(readFileSync(failed.evidenceFile, "utf8"), /Test rejected C/);
  assert.equal(git(input.repo, "rev-parse", run.integrationRef), before);
  const drift = await landNode(run, c, async () => {
    git(input.repo, "update-ref", run.integrationRef, nodeCommit, before);
    return passed();
  });
  assert.equal(drift.outcome, "ref_drift");
  assert.equal(git(input.repo, "rev-parse", run.integrationRef), nodeCommit);
});
test("cleanup requires confirmation and only removes a clean worktree owned by the current run", () => {
  ...
  git(node.dir, "add", ".");
  git(node.dir, "commit", "-qm", "Keep work");
  cleanupNodeWorkspace(run, "node", { confirmed: true });
  assert.equal(existsSync(node.dir), false);
  assert(existsSync(input.repo));
  assert.equal(git(input.repo, "rev-parse", "HEAD"), input.baseCommit);
});
test("landing rejects verification that changes the candidate or its owned integration checkout", async () => {
  ...
  assert.equal(changed.outcome, "verification_failed");
  ...
  assert.equal(
    readFileSync(path.join(run.integrationDir, "base.txt"), "utf8"),
...
test("Git metadata outside the workspace cannot receive run refs", () => {
  const input = setup();
  const outside = mkdtempSync(path.join(tmpdir(), "plan-git-outside-"));
  const moved = path.join(outside, "git");
  renameSync(path.join(input.repo, ".git"), moved);
  symlinkSync(moved, path.join(input.repo, ".git"));
...
test("landing refuses integration branch attachment and detached HEAD drift before updating refs", async () => {
  for (const change of ["attach", "head"]) {
    ...
    assert(existsSync(result.evidenceFile));
    assert.equal(git(input.repo, "rev-parse", "HEAD"), input.baseCommit);
    assert.equal(git(input.repo, "status", "--porcelain"), "");
    assert.equal(git(input.repo, "rev-parse", run.integrationRef), input.baseCommit);
  }
...
test("authorized integration alignment checks both heads and only updates its clean detached checkout", () => {
  ...
  git(run.integrationDir, "add", ".");
  git(run.integrationDir, "commit", "-qm", "Preserved local work");
...
test("failures after integration CAS require inspection and retain candidate verification evidence", async () => {
  for (const failure of ["reset", "receipt"]) {
    ...
    await assert.rejects(
      landNode(run, node, async (candidate) => {
        if (failure === "reset")
          writeFileSync(
            git(run.integrationDir, "rev-parse", "--git-path", "index.lock"),
            "Fixture stale lock",
          );
        else
          writeFileSync(
            path.join(path.dirname(candidate.dir), "landing.json"),
            "Existing receipt must not be overwritten",
          );
        return passed();
      }),
      (error: unknown) => {
        assert(error instanceof Error);
        assert.equal(error.name, "LandingUncertainError");
        confirmation = (error as LandingUncertainError).confirmation;
        assert.equal(confirmation.baseCommit, input.baseCommit);
        assert.equal(
          git(input.repo, "rev-parse", run.integrationRef),
          confirmation.candidateCommit,
        );
        assert(existsSync(confirmation.candidateDir));
        assert(existsSync(confirmation.verificationFile));
    ...
    if (failure === "receipt")
      assert.equal(
        readFileSync(confirmation.evidenceFile, "utf8"),
        "Existing receipt must not be overwritten",
      );
  }
});
