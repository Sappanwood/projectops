...
test("a ready parallel run blocks a competing serial owner before either dispatches", () => {
  const workspaceDir = mkdtempSync(path.join(tmpdir(), "run-modes-"));
  ...
  const root = path.join(workspaceDir, "ops/repo/plans");
  writeFileSync(
    path.join(root, "plan-owner.json"),
    JSON.stringify({
      schema: "plan/Plan@1",
      id: "plan-owner",
      title: "Owner",
  ...
  cli(["plan", "materialize", "repo", "plan-owner"]);
  const q = {
    workspaceDir,
    projectId: "repo",
    planId: "plan-owner",
    expectedRevision: computePlanRevision(readPlan(root, "plan-owner")),
  };
  ...
  assert.equal(parallel.ok, true, JSON.stringify(parallel));
  assert.equal(createPlanRun(q).ok, false);
  cli(["backlog", "update", "repo", "REP-001", "--status", "done"]);
  const input = {
    workspaceRoot: workspaceDir,
    plansRoot: root,
    backlogRoot: path.join(workspaceDir, "ops/repo/backlog"),
    reportsRoot: path.join(workspaceDir, "ops/repo/reports"),
    projectId: "repo",
    planId: "plan-owner",
