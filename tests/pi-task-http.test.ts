...
test("HTTP steer requires same origin, JSON, exact revision and a message; persists accepted instructions", async () => {
  ...
  try {
    const started = await (
      await post("start", { item_id: item.id, expected_revision: item.revision })
    ).json();
    ...
    assert.ok(
      detail.data.attempt.progress.events.some((event: { text: string }) =>
        event.text.includes(input.message),
      ),
    );
  } finally {
