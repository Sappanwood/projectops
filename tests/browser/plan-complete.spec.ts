...
test('explicit Web completion handles stale revisions and persists across refresh and CLI reads', async ({ workbench, page }) => {
  const input = path.join(workbench.root, 'complete.json');
  const draft = { title: 'Complete in Web', goal: 'Deliver work', items: [{ key: 'task', title: 'Task', item_type: 'task', priority: 'P1', body: 'Acceptance' }] };
  ...
  workbench.cli(['backlog', 'update', 'alpha', mapping.task, '--status', 'done', '--expected-revision', item.revision]);
  ...
  const args = ['plan', 'revise', 'alpha', id, '--input', input, '--expected-revision', current.revision];
  ...
  expect(JSON.parse(workbench.cli(['plan', 'show', 'alpha', id, '--json'])).status).toBe('approved');
  await page.getByRole('button', { name: 'Refresh workspace and project data' }).click();
  await complete.click();
