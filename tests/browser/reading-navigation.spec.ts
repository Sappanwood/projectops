...
test("Report links navigate to Docs and Backlog and return to the report", async ({
  workbench,
  page,
}) => {
...
test("Retrospective detail and filters survive reload and task round trips", async ({
  workbench,
  page,
}) => {
  const before = workbench.snapshot();
  await page.goto(
    `${workbench.origin}/#/projects/alpha/retrospectives/browser?filter_project=alpha&status=inbox&task=ALP-001`,
  );
