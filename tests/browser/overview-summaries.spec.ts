...
test("Overview counts active previews and refreshes live progress without changing report outcomes", async ({workbench,page}) => {
  for (let i = 0; i < 6; i++) workbench.cli(['backlog','add','alpha','-T',`Pending ${i}`,'-c','feature','--priority','P1']);
  ...
  for (let i = 1; i <= 7; i++) workbench.cli(['backlog','update','alpha',`ALP-${String(i).padStart(3,'0')}`,'--status','done']);
...
test("Overview limits Plan previews and distinguishes domain errors from empty lists", async ({workbench,page}) => {
  for (let i = 0; i < 6; i++) writeFileSync(path.join(workbench.root,`ops/alpha/plans/plan-new-${i}.json`),JSON.stringify({
    schema:'plan/Plan@1',id:`plan-new-${i}`,title:`Draft ${i}`,goal:'Fixture',status:'draft',
    items:[{key:'one',title:'Fixture',item_type:'task',priority:'P1',body:'Fixture',depends_on:[]}],
  }));
