...
test("Overview details support direct URLs, reloads, new tabs and return navigation", async ({ workbench, page, context }) => {
  ...
  await page.goto(`${workbench.origin}/#/projects/alpha/backlog/ALP-999?from=${encodeURIComponent('#/projects/alpha')}`);
...
test("Overview restores its scroll position after explicit return and browser back", async ({workbench,page}) => {
