...
test('Plan run waits for acceptance, preserves pause across reload and explicitly resumes its dependent task', async ({ workbench, page }) => {
  await workbench.stop();
  execFileSync('git', ['init', '-q'], { cwd: path.join(workbench.root, 'alpha') });
  const draft = path.join(workbench.root, 'serial-browser.json');
  writeFileSync(draft, JSON.stringify({ title: 'Serial browser', goal: 'Run tasks in order', items: [1, 2].map(n => ({ key: `step${n}`, title: `Serial task ${n}`, item_type: 'task', priority: 'P1', body: 'Implement and verify', depends_on: n === 1 ? [] : ['step1'] })) }));
  ...
  let starts = 0, stops = 0, attemptId = ''; let finish!: (result: RunnerResult) => void;
  const server = await startWorkbenchServer({ workspaceDir: workbench.root, port: 0, staticDir: path.resolve('dist/web'), runner: {
    start(attempt) { starts++; attemptId = attempt.id; return { completion: new Promise(resolve => { finish = resolve; }), stop() { stops++; } }; },
  } });
  ...
  try {
    ...
    const verified = verifyExecution({ ...query, expectedRevision: current.data.attempt.revision, command: 'serial fixture checks', outcome: 'passed', evidence: 'First task checks passed' }); expect(verified.ok).toBe(true);
