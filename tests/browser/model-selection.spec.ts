...
test("header selection persists, refreshes local availability, and leaves active work unchanged", async ({
  workbench,
  page,
}) => {
  const one = { provider: "fixture", id: "one" },
    two = { provider: "fixture", id: "two" };
  let models = [
    { ...one, name: "One" },
    { ...two, name: "Two" },
  ];
  ...
  const server = await startWorkbenchServer({
    ...
    runner: {
      ...
      start(attempt, context) {
        received.push(attempt);
        context.recordModel?.(attempt.input.model!);
        return {
  ...
  try {
    ...
    await expect(page.locator("#model-selection-status")).toHaveText(
      "所选模型不可用，请刷新或重新选择",
    );
