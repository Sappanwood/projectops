...
test("Overview keeps long titles, IDs and metadata readable at every viewport", async ({
  workbench,
  page,
}, testInfo) => {
  ...
  for (const width of [375, 768, 1280, 1440]) {
    ...
    const overflow = await page
      .locator(".overview-card, .overview-card .item-title, .workbench-header, .project-nav")
