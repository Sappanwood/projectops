...
test("Docs reads standard and additional documents, headings and source without writes", async ({
  workbench,
  page,
}, testInfo) => {
  mkdirSync(path.join(workbench.root, "alpha/docs/guide"), { recursive: true });
  writeFileSync(
    path.join(workbench.root, "alpha/README.md"),
    "# 项目阅读\n\n[使用指南](docs/guide/use.md#步骤)\n\n| 名称 | 结果 |\n| --- | --- |\n| 检查 | passed |\n",
  );
  writeFileSync(
    path.join(workbench.root, "alpha/docs/guide/use.md"),
    "# 使用指南\n\n[概览](../../README.md)\n\n## 步骤\n\n- [x] 已完成\n\n> 注意阅读\n",
  );
...
test("Docs retries failed reads, shows missing documents and discards late project responses", async ({
  workbench,
  page,
}) => {
  await page.route("**/api/projects/alpha/docs?*", (route) =>
    route.fulfill({
      status: 503,
      contentType: "application/json",
      body: JSON.stringify({ ok: false, error: { message: "Temporary read failure" } }),
    }),
  );
  await page.goto(`${workbench.origin}/#/projects/alpha/docs`);
  ...
  await page.route("**/api/projects/alpha/docs?*", async (route) => {
    started();
    await pending;
...
test("Document navigation restores the reading position with browser back and forward", async ({
  workbench,
  page,
}) => {
  const body =
