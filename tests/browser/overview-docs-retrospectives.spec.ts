...
test("Overview standard documents open directly, reload and return while unreadable entries explain failures", async ({workbench,page,context}, testInfo) => {
  ...
  for (const name of ['README.md','AGENTS.md','docs/PRODUCT_SPEC.md','docs/ARCHITECTURE.md']) {
    ...
    const link = page.locator('[aria-labelledby="card-docs-title"]').getByRole('link',{name,exact:true});
    await link.focus();
  ...
  writeFileSync(path.join(workbench.root,'alpha/docs/PRODUCT_SPEC.md'),'Readable without heading');
  await page.goto(overview);
...
test("Overview shows readable retrospective previews, scoped counts and partial diagnostics", async ({workbench,page}) => {
  const root = path.join(workbench.root,'retrospectives/inbox');
  const body = readFileSync(path.join(root,'browser.md'),'utf8');
  for (let i = 0; i < 6; i++) writeFileSync(path.join(root,`preview-${i}.md`),body.replace('id: browser',`id: preview-${i}`).replace('2026-09-05',`2026-09-0${i+1}`).replace('Browser retrospective body',`Readable preview ${i}`));
  ...
  await expect(retro.locator('.item-row')).not.toContainText(['inbox/','inbox/','inbox/','inbox/','inbox/']);
  await retro.locator('.item-title').first().click();
  await expect(page.locator('[data-retrospective-id="preview-5"] .markdown-content')).toContainText('Readable preview 5');
});
