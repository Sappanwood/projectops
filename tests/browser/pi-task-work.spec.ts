...
test("task work streams progress, preserves draft and focus, steers, reloads and requires explicit acceptance", async ({
  workbench,
  page,
}) => {
  ...
  try {
    ...
    expect(
      JSON.parse(workbench.cli(["backlog", "show", "alpha", "ALP-001", "--json"])).status,
    ).not.toBe("done");
    const query = { workspaceDir: workbench.root, projectId: "alpha", attemptId };
    const shown = showExecution(query);
    expect(shown.ok).toBe(true);
    if (!shown.ok) return;
    const verified = verifyExecution({
      ...query,
      expectedRevision: shown.data.attempt.revision,
      command: "controlled assertions",
      outcome: "passed",
      evidence: "Controlled assertions passed <script>unsafe</script>",
    });
    expect(verified.ok).toBe(true);
    await panel.getByRole("button", { name: "刷新执行记录" }).click();
    await panel.getByRole("button", { name: "查看验证证据正文" }).click();
    await expect(panel.getByLabel("验证证据正文")).toHaveText(
      "Controlled assertions passed <script>unsafe</script>",
    );
    await expect(panel.locator("script")).toHaveCount(0);
    ...
    if (verified.ok)
      writeFileSync(
        path.join(
          workbench.root,
          "ops/alpha/executions",
          verified.data.attempt.verifications[0]!.evidence_ref,
        ),
        "Tampered evidence",
      );
    await panel.getByRole("button", { name: "查看验证证据正文" }).click();
...
test("repeated start clicks launch once and failed work waits for explicit retry with history preserved", async ({
  workbench,
  page,
}) => {
  execFileSync("git", ["init", "-q"], { cwd: path.join(workbench.root, "alpha") });
