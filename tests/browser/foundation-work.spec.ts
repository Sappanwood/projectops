...
test('task edits preserve conflicting input and Plan revisions require preview', async ({ workbench, page }) => {
  ...
  workbench.cli(['backlog','update','alpha','ALP-001','--status','in_progress','--expected-revision',item.revision]);
  ...
  expect(JSON.parse(workbench.cli(['backlog','show','alpha','ALP-001','--json'])).title).toBe('Clarified browser task');
...
test('execution evidence supports explicit acceptance after page reload', async ({ workbench, page }) => {
  const { execFileSync } = await import('node:child_process');
  const { createExecution, finishExecution, verifyExecution } = await import('../../src/application/executionApi.js');
  ...
  const created = createExecution(query); expect(created.ok).toBe(true); if (!created.ok) return;
  const first = created.data.attempt;
  const ended = finishExecution({ ...query, attemptId: first.id, expectedRevision: first.revision, outcome: 'succeeded', summary: 'Browser implementation evidence' });
  expect(ended.ok).toBe(true); if (!ended.ok) return;
  const verified = verifyExecution({ ...query, attemptId: first.id, expectedRevision: ended.data.attempt.revision, command: 'fixture assertions', outcome: 'passed', evidence: 'All assertions passed' });
  ...
  const reverified = verifyExecution({ ...query, attemptId: first.id, expectedRevision: verified.data.attempt.revision, command: 'recheck', outcome: 'passed', evidence: 'Checked again' });
...
test('controlled runner survives reload, separates stop request and confirmation, and records a retry', async ({ workbench, page }) => {
  const { execFileSync } = await import('node:child_process');
  ...
  const server = await startWorkbenchServer({ workspaceDir: workbench.root, port: 0, staticDir: path.resolve('dist/web'), runner: {
    start() { starts++; return { completion: new Promise(resolve => { finish = resolve; }), stop() { stopRequested = true; } }; },
  } });
  try {
...
test('failed verification and missing evidence remain visible; rework preserves the decision', async ({ workbench, page }) => {
  const { execFileSync } = await import('node:child_process');
  ...
  const query = { workspaceDir: workbench.root, projectId: 'alpha', itemId: 'ALP-001' };
  const created = createExecution(query); expect(created.ok).toBe(true); if (!created.ok) return;
  const ended = finishExecution({ ...query, attemptId: created.data.attempt.id, expectedRevision: created.data.attempt.revision, outcome: 'succeeded', summary: 'Needs more work' });
  expect(ended.ok).toBe(true); if (!ended.ok) return;
  const verified = verifyExecution({ ...query, attemptId: ended.data.attempt.id, expectedRevision: ended.data.attempt.revision, command: 'failed check', outcome: 'failed', evidence: 'Regression remains' });
  expect(verified.ok).toBe(true); if (!verified.ok) return;
  rmSync(`${workbench.root}/ops/alpha/executions/${verified.data.attempt.verifications[0]!.evidence_ref}`);
  await page.goto(`${workbench.origin}/#/projects/alpha/backlog/ALP-001`);
  const panel = page.getByRole('region', { name: '任务执行' });
...
test('orphaned runtime work requires an explicit inspection before interruption confirmation', async ({ workbench, page }) => {
  const { createExecution } = await import('../../src/application/executionApi.js');
  const { startWorkbenchServer } = await import('../../src/server/workbenchServer.js');
  const path = await import('node:path');
  const created = createExecution({ workspaceDir: workbench.root, projectId: 'alpha', itemId: 'ALP-001', origin: 'runtime' });
  expect(created.ok).toBe(true);
  const server = await startWorkbenchServer({ workspaceDir: workbench.root, port: 0, staticDir: path.resolve('dist/web') });
  try {
    await page.goto(`${server.origin}/#/projects/alpha/backlog/ALP-001`);
    const panel = page.getByRole('region', { name: '任务执行' });
