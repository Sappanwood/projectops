...
test("Plan lifecycle shows no report, partial and completed snapshots with return navigation", async ({ workbench, page }) => {
  const input = path.join(workbench.root, "delivery.json");
  writeFileSync(input, JSON.stringify({ title: "Final delivery", goal: "Full workflow", items: [
    { key: "first", title: "First delivery task", item_type: "task", priority: "P1", body: "First scope" },
    { key: "second", title: "Second delivery task", item_type: "task", priority: "P1", body: "Second scope", depends_on: ["first"] },
  ] }));
  workbench.cli(["plan", "create", "alpha", "--input", input]);
  workbench.cli(["plan", "approve", "alpha", "plan-final-delivery", "--review-note", "Fixture approval"]);
  const { mapping } = JSON.parse(workbench.cli(["plan", "materialize", "alpha", "plan-final-delivery", "--json"]));
  ...
  const update = (id: string, status: string) => {
    ...
    workbench.cli(["backlog", "update", "alpha", id, "--status", status, "--expected-revision", item.revision]);
  };
  update(mapping.first, "done");
  workbench.cli(["report", "create", "alpha", "plan-final-delivery", "--report-id", "report-partial", "--verification", "Fixture first task complete", "--partial-acceptance", "Fixture accepts pending second task"]);
  ...
  workbench.cli(["report", "create", "alpha", "plan-final-delivery", "--report-id", "report-completed", "--verification", "Fixture all tasks complete"]);
