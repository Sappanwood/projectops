...
test("Backlog done cannot produce completed Report while its real run is unconfirmed", () => {
  ...
  const cli = (a: string[]) =>
    assert.equal(runCli(a, { stdout() {}, stderr() {} }, workspaceRoot), 0);
  cli(["init"]);
  ...
  writeFileSync(
    path.join(plansRoot, "plan-proof.json"),
    JSON.stringify({
      schema: "plan/Plan@1",
      id: "plan-proof",
      title: "Proof",
      goal: "Verify",
      status: "approved",
      approval: { approved_at: new Date().toISOString(), review_note: "fixture" },
      items: [
        { key: "a", title: "A", item_type: "task", priority: "P1", body: "Verify", depends_on: [] },
      ],
    }),
  );
  cli(["plan", "materialize", "repo", "plan-proof"]);
  const created = createPlanRun({
    workspaceDir: workspaceRoot,
    projectId: "repo",
    planId: "plan-proof",
    expectedRevision: computePlanRevision(readPlan(plansRoot, "plan-proof")),
  });
  assert.equal(created.ok, true);
  cli(["backlog", "update", "repo", "REP-001", "--status", "done"]);
  const input = {
    workspaceRoot,
    reportsRoot: path.join(workspaceRoot, "ops/repo/reports"),
    backlogRoot: path.join(workspaceRoot, "ops/repo/backlog"),
    plansRoot,
    projectId: "repo",
    planId: "plan-proof",
    verification: ["fixture evidence"],
  };
  assert.equal(
    completePlan({
      workspaceDir: workspaceRoot,
  ...
  const partial = writeGeneratedReport({
    ...input,
    partialAcceptance: "Fixture explicitly accepts pending run.",
  });
