...
- Workbench 项目栏直接显示项目 ID，按 ID 固定排序并高亮当前项目；使用原生链接支持键盘导航和新标签页。宽屏在左侧排列项目与功能导航，窄屏以两行展示；切换项目保留当前栏目、清除原项目详情和过滤条件。窄屏在项目栏内横向滚动，首次加载、切换项目和窗口缩窄时露出当前项目；刷新保留栏内滚动位置。
- 全站采用浅色文档审阅工作台布局，使用 React 稳定页面与导航节点。Overview 摘要优先于开发服务控制；Backlog 任务正文优先于依赖和执行面板；Docs／Research 提供正文与目录；Reports／Retrospectives 保留折叠详情与筛选。普通报告／回顾刷新保留未变化正文的选区，读取失败明确提示当前内容为上次结果并提供重试。
- Plan 列表按草案、已批准、已完成分组，草案优先，同组按 ID 排序；状态计数保留，空分组不占正文空间。页面采用浅色文档审阅布局，宽屏使用左侧项目／功能导航，详情右侧显示计划状态、任务进度、报告数量与审阅提示；窄屏导航横向滚动、状态栏移至正文后。详情独立展示单份计划，默认“审阅计划” Tab，完整目标与默认展开的任务正文支持连续阅读；提供任务目录及全部展开/折叠，窄屏目录按需打开。刷新保留未变化正文的选区；计划状态、执行进度和验收结果分别展示。
...
“审阅计划”在目标下方展示计划声明的依赖图，箭头从前置任务指向后续任务，无依赖任务也保留。
节点仅显示代号：已生成条目使用 Backlog ID，跨项目条目带项目限定；尚未生成的任务使用 Plan key。
悬停或键盘聚焦显示标题，点击已生成条目进入真实项目的 Backlog 并保留返回审阅页的入口；草案及部分物化中
尚未创建的节点展开并定位计划正文。既有任务以虚线边框区分，按完整项目身份去重，不计入 Plan 任务范围。
既有依赖的总览集中在图中；各任务正文继续保留前置依赖与下游说明，方便就近阅读。
既有任务标题按需读取；加载与失败均保留节点和链接，失败后重新悬停或聚焦可重试。
图支持局部滚动并在页面刷新时保留滚动位置；它展示 Plan 声明，不替代物化后 Backlog 的实时依赖与执行就绪判断。
...
App 外 Agent 直接读取 Plan 文件，或通过 `pops plan show <project> <plan-id> --json` 获取计划内容与 revision。
