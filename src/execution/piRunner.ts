    ...
    const completion = (async (): Promise<RunnerResult> => {
      try {
        ...
        unsubscribe = session.subscribe(raw => {
          const event = raw as { type?: string; toolName?: string; assistantMessageEvent?: { type: string; delta?: string } };
          if (event.type === 'message_update' && event.assistantMessageEvent?.type === 'text_delta')
            context.emit({ type: 'text', text: event.assistantMessageEvent.delta ?? '' });
          if (event.type === 'tool_execution_start' || event.type === 'tool_execution_end')
            context.emit({ type: 'tool', text: `${event.toolName ?? 'tool'}: ${event.type === 'tool_execution_start' ? '开始' : '结束'}` });
        });
        if (stopped) return { outcome: 'stopped', summary: 'Pi 已在开始前停止。' };
        await session.prompt(`完成以下 ProjectOps 任务。遵守已加载的 AGENTS.md 与任务边界。不要修改任务状态、执行记录或自行验收，不提交或发布 Git 改动。不要读取、输出或写入凭据。结束时说明改动和实际验证结果。\n\n${attempt.input.item.id}: ${attempt.input.item.title}\n\n${attempt.input.item.body}\n\n补充指示：\n${attempt.input.instructions}`);
        const last = [...session.messages].reverse().find(m => (m as {role?:string}).role === 'assistant') as {stopReason?:string;content?:{type:string;text?:string}[]} | undefined;
        if (stopped || last?.stopReason === 'aborted') return { outcome: 'stopped', summary: 'Pi 已确认停止。' };
        if (last?.stopReason !== 'stop') return { outcome: 'failed', summary: 'Pi 未正常完成响应（错误、截断或结果缺失），请检查后重试。' };
        const summary = last?.content?.filter(c => c.type === 'text').map(c => c.text ?? '').join('\n');
        return { outcome: 'succeeded', summary: summary || 'Pi 工作结束，仍需记录验证并显式验收。' };
      } catch (error) {
        return { outcome: stopped ? 'stopped' : 'failed', summary: stopped ? 'Pi 已停止。' : error instanceof PiConfigurationError ? error.message : 'Pi 启动或工作失败，请检查服务端模型、凭据和连接配置后重试。' };
      } finally { unsubscribe?.(); session?.dispose(); }
    })();
    return { completion, async steer(message) { const current = await ready; if (stopped) throw Error('Pi is stopping.'); await current.steer(message); },
      async stop() { stopped = true; const current = await ready; current.clearQueue(); await current.abort(); } };
  } };
}
...
async function openPiSession(attempt: ExecutionAttempt, context: StartContext): Promise<PiSession> {
  ...
  for (const segment of ['.pops', 'runtime', 'pi', attempt.id]) {
    ...
    try { mkdirSync(sessionDir); } catch (error) { if ((error as NodeJS.ErrnoException).code !== 'EEXIST') throw error; }
