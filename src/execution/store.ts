...
export function context(workspaceDir: string, projectId: string) {
  ...
  if (w.manifest.artifact_layout.roots.executions !== EXECUTION_SCHEMA)
    throw new ExecutionError(
      "EXECUTION_INVALID",
      "Execution artifact descriptor is missing or invalid.",
    );
  ...
  if (!isWithinWorkspace(realpathSync(w.root), realpathSync(roots.executions)))
    throw new ExecutionError("EXECUTION_INVALID", "Execution root resolves outside workspace.");
  const repo = resolveProjectPath(w.root, p.path);
  if (!isWithinWorkspace(realpathSync(w.root), realpathSync(repo)))
    throw new ExecutionError("EXECUTION_INVALID", "Repository resolves outside workspace.");
  return {
    workspace: w.root,
    root: roots.executions,
    backlog: roots.backlog,
    plans: roots.plans,
    repo: resolveProjectPath(w.root, p.path),
  };
}
export function readAttempt(root: string, attemptId: string, projectId?: string): ExecutionAttempt {
  ...
  if (!existsSync(file)) throw new ExecutionError("EXECUTION_NOT_FOUND", "Attempt not found.");
  if (!isWithinWorkspace(realpathSync(root), realpathSync(file)))
    throw new ExecutionError("EXECUTION_INVALID", "Attempt resolves outside execution root.");
  ...
  if (
    a.schema !== EXECUTION_SCHEMA ||
    a.id !== attemptId ||
    !Array.isArray(a.verifications) ||
    !a.input?.item
  )
    throw new ExecutionError("EXECUTION_INVALID", "Unsupported or invalid execution record.");
  if (
    !["running", "stop_requested", "unknown", "succeeded", "failed", "stopped"].includes(a.state) ||
    typeof a.revision !== "string" ||
    !/^[a-f0-9]{16}$/.test(a.revision) ||
    typeof a.started_at !== "string" ||
    !Number.isFinite(Date.parse(a.started_at)) ||
    !["external", "runtime"].includes(a.origin) ||
    typeof a.project_id !== "string" ||
    a.input.item.project !== a.project_id ||
    a.input.item.id !== a.item_id ||
    (projectId !== undefined && a.project_id !== projectId) ||
    (a.acceptance !== null &&
      (!a.acceptance ||
        !["accepted", "rework"].includes(a.acceptance.decision) ||
        typeof a.acceptance.note !== "string" ||
        typeof a.acceptance.at !== "string" ||
        !Number.isFinite(Date.parse(a.acceptance.at)))) ||
    a.verifications.some(
      (v) =>
        !v ||
        !["passed", "failed"].includes(v.outcome) ||
        typeof v.command !== "string" ||
        typeof v.evidence_ref !== "string" ||
        typeof v.evidence_digest !== "string" ||
        !/^[a-f0-9]{64}$/.test(v.evidence_digest) ||
        !v.snapshot?.digest,
    )
  )
  ...
  if (
    ...
    (!a.checkout ||
      !/^run-[a-f0-9-]{36}$/.test(a.checkout.run_id) ||
      !/^[a-zA-Z0-9][a-zA-Z0-9_-]{0,80}$/.test(a.checkout.node_id))
  )
    throw new ExecutionError("EXECUTION_INVALID", "Execution checkout reference is invalid.");
  if (
    a.progress !== undefined &&
    (!a.progress ||
      !Array.isArray(a.progress.events) ||
      (a.progress.session_id !== undefined && typeof a.progress.session_id !== "string") ||
      a.progress.events.some(
        (e) =>
          !e ||
          !["text", "tool", "status", "session"].includes(e.type) ||
...
export function listAttempts(root: string, projectId?: string) {
  return readdirSync(root)
    .filter((f) => /^exe-.*\.json$/.test(f))
    .map((f) => readAttempt(root, f.slice(0, -5), projectId))
    .sort((a, b) => a.started_at.localeCompare(b.started_at));
}
export function saveAttempt(root: string, a: ExecutionAttempt, create = false) {
  a.revision = createHash("sha256")
    .update(JSON.stringify({ ...a, revision: "" }))
    .digest("hex")
    .slice(0, 16);
...
export function saveProgress(
  ...
  progress: NonNullable<ExecutionAttempt["progress"]>,
) {
...
export function evidence(root: string, attemptId: string, body: string) {
  ...
  if (!isWithinWorkspace(realpathSync(root), realpathSync(dir)))
    throw new ExecutionError(
      "EXECUTION_INVALID",
      "Evidence directory resolves outside execution root.",
    );
  const ref = `evidence/${attemptId}-${randomUUID()}.txt`;
  writeFileSync(path.join(root, ref), body, { flag: "wx" });
  return ref;
}
export function evidenceDigest(body: string) {
  return createHash("sha256").update(body).digest("hex");
}
export function evidenceReadable(root: string, ref: string, digest: string) {
  if (!/^evidence\/exe-[a-f0-9-]+\.txt$/.test(ref)) return false;
  try {
    const file = path.join(root, ref);
    const body = readFileSync(file, "utf8");
    return (
      isWithinWorkspace(realpathSync(root), realpathSync(file)) &&
      body.trim().length > 0 &&
...
export function validateAcceptanceTargets(c: ReturnType<typeof context>, itemId: string) {
  ...
  if (
    !isWithinWorkspace(workspace, backlog) ||
    !isWithinWorkspace(backlog, items) ||
    !isWithinWorkspace(items, item) ||
    !isWithinWorkspace(backlog, index)
  ) {
    throw new ExecutionError(
      "EXECUTION_INVALID",
      "Acceptance targets resolve outside their declared backlog store.",
    );
  }
}

export function executionRepo(
  c: ReturnType<typeof context>,
  a: Pick<ExecutionAttempt, "checkout">,
): string {
  return a.checkout
    ? readNodeWorkspace(c.workspace, c.repo, a.checkout.run_id, a.checkout.node_id).node.dir
    : c.repo;
}
