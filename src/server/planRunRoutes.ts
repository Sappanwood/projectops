...
export async function handlePlanRunRoute(
  segments: string[],
  method: string | undefined,
  workspaceDir: string,
  runtime: PlanRunRuntime,
  http: Http,
): Promise<boolean> {
  if (segments[0] !== "api" || segments[1] !== "projects" || segments[3] !== "plan-runs")
    return false;
  const input = { workspaceDir, projectId: segments[2]! };
  if (segments.length === 4 && method === "GET") {
    const planId = http.query("plan_id");
    http.send(listPlanRuns({ ...input, ...(planId ? { planId } : {}) }));
    return true;
  }
  http.noQuery();
    ...
    if (body.reuse !== undefined) {
      ...
      reuse = body.reuse.map((value) => {
        if (
          ...
          typeof value !== "object" ||
          Array.isArray(value) ||
          Object.keys(value).some((key) => !["item_id", "attempt_id", "note"].includes(key)) ||
          typeof value.item_id !== "string" ||
          typeof value.attempt_id !== "string" ||
          typeof value.note !== "string"
        )
          http.invalid("Reuse entry is invalid.");
  ...
  if (segments.length === 5) {
    http.method("GET");
    http.send(showPlanRun({ ...input, runId: segments[4]! }));
    return true;
  }
  if (
    segments.length === 6 &&
    ["advance", "pause", "resume", "stop-current", "close-stopped"].includes(segments[5]!)
  ) {
    http.method("POST");
    const action = segments[5]!;
    const body = await http.body(
      action === "resume"
        ? ["expected_revision", "note", "baseline_digest"]
    ...
    const q = {
      ...
      ...(typeof body.baseline_digest === "string" ? { baselineDigest: body.baseline_digest } : {}),
    };
    http.send(
      action === "advance"
        ? await runtime.advance(q)
        : action === "pause"
          ? await runtime.pause(q)
          : action === "resume"
            ? await runtime.resume({ ...q, note: typeof body.note === "string" ? body.note : "" })
            : action === "close-stopped"
              ? await runtime.closeStopped({
                  ...q,
                  note: typeof body.note === "string" ? body.note : "",
                })
              : await runtime.stopCurrent(q),
    );
    return true;
  }
  return false;
