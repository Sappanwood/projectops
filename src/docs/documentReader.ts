...
export type DocumentList = {
  ...
  diagnostics: Array<{ path: string; issue: string }>;
};
export type ProjectDocument = { path: string; body: string };
export class DocumentReadError extends Error {
  constructor(
    public readonly code: "DOCUMENT_NOT_FOUND" | "DOCUMENT_UNAVAILABLE" | "INVALID_DOCUMENT_PATH",
    message: string,
  ) {
    super(message);
  }
...
export function isDocumentPath(value: string): boolean {
  ...
  return (
    ...
    (value === "README.md" ||
      value === "AGENTS.md" ||
      (parts[0] === "docs" && parts.length > 1 && /\.md$/i.test(value)))
  );
}
...
function projectRoot(workspace: string, project: string): string {
  ...
  if (!within(realpathSync(workspace), root) || !lstatSync(root).isDirectory())
    throw new DocumentReadError("DOCUMENT_UNAVAILABLE", "项目文档目录不可用。");
  return root;
}
function regularTarget(root: string, relative: string): string {
  ...
  for (const [index, part] of parts.entries()) {
    ...
    if (
      stat.isSymbolicLink() ||
      !within(root, realpathSync(current)) ||
...
function readError(error: unknown): DocumentReadError {
  ...
  if (typeof error === "object" && error !== null && "code" in error && error.code === "ENOENT")
    return new DocumentReadError("DOCUMENT_NOT_FOUND", "文档不存在。");
...
export function readProjectDocument(
  ...
  if (!isDocumentPath(documentPath))
    throw new DocumentReadError(
      ...
      "仅支持 README.md、AGENTS.md 和 docs/ 下的 Markdown 文档。",
    );
  try {
  ...
  const documents: DocumentSummary[] = PROJECT_DOC_TEMPLATES.map(({ path: documentPath }) => {
    try {
      ...
      return {
        path: documentPath,
        standard: true,
        issue: hasLevelOneHeading(document.body) ? null : "缺少一级 Markdown 标题；仍可阅读。",
      };
    } catch (error) {
      return { path: documentPath, standard: true, issue: readError(error).message };
    }
  });
  ...
  function walk(relative: string): void {
    try {
      ...
      if (stat.isSymbolicLink() || !stat.isDirectory() || !within(root, realpathSync(absolute))) {
        diagnostics.push({ path: relative, issue: "目录不可读取（非普通目录或符号链接）。" });
        return;
      }
      for (const entry of readdirSync(absolute, { withFileTypes: true }).sort((a, b) =>
        ...
        if (entry.isDirectory()) walk(name);
          ...
          documents.push({
            path: name,
            standard: false,
            issue: entry.isFile() ? null : "文档不是普通文件。",
          });
        }
      }
    } catch (error) {
