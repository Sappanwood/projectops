...
export function executionResult<T>(f: () => T): ApplicationResult<T> {
  try {
    return applicationSuccess(f());
  } catch (e) {
    return e instanceof ExecutionError
      ? applicationFailure(e.code, e.message)
      : applicationFailure("EXECUTION_INVALID", "Execution data or workspace is unavailable.");
  }
}
function detail(root: string, attempt: ExecutionAttempt): AttemptDetail {
  return {
    attempt,
    diagnostics: attempt.verifications
      .filter((v) => !evidenceReadable(root, v.evidence_ref, v.evidence_digest))
      .map((v) => `Evidence unavailable: ${v.evidence_ref}`),
  };
}
export function listExecutions(q: ExecutionQuery) {
  return executionResult(() => {
    const c = context(q.workspaceDir, q.projectId);
    return {
      attempts: listAttempts(c.root, q.projectId).filter(
        (a) => !q.itemId || a.item_id === q.itemId,
      ),
    };
  });
}
export function showExecution(q: AttemptQuery) {
  return executionResult(() => {
    const c = context(q.workspaceDir, q.projectId);
    const a = readAttempt(c.root, q.attemptId, q.projectId);
    if (q.itemId && a.item_id !== q.itemId)
      throw new ExecutionError("EXECUTION_NOT_FOUND", "Attempt does not belong to this task.");
    return detail(c.root, a);
  });
}
export function createExecution(
  ...
  return executionResult(() => {
    ...
    if (!shown.ok) throw new ExecutionError("EXECUTION_INVALID", shown.error.message);
    ...
    if (item.status === "done" || item.status === "cancelled")
      throw new ExecutionError(
        "EXECUTION_CONFLICT",
        "Completed or cancelled task cannot start work.",
      );
    const previous = q.retryOf ? readAttempt(c.root, q.retryOf, q.projectId) : undefined;
    if (
      previous &&
      (previous.item_id !== q.itemId ||
        activeStates.includes(previous.state) ||
        (previous.acceptance?.decision === "accepted" && !(checkout && previous.checkout)))
    )
      throw new ExecutionError(
        "EXECUTION_CONFLICT",
        "Retry requires an ended, unaccepted attempt of this task.",
      );
    if (attempts.length && !previous)
      throw new ExecutionError(
        "EXECUTION_CONFLICT",
        "Existing history requires an explicit retryOf attempt.",
      );
    ...
    const attempt: ExecutionAttempt = {
      ...
      id: id(),
      execution_id: previous?.execution_id ?? id(),
      retry_of: previous?.id ?? null,
      project_id: q.projectId,
      item_id: q.itemId,
      task_ref: `project-ops:backlog/items/${q.itemId}.md`,
      revision: "",
      origin: q.origin ?? "external",
      input: { item: structuredClone(item), instructions: q.instructions ?? "", plan },
...
export function mutateExecution(
  q: AttemptMutation,
  f: (a: ExecutionAttempt, c: ReturnType<typeof context>) => void | true,
) {
  return executionResult(() => {
    const c = context(q.workspaceDir, q.projectId);
    const a = readAttempt(c.root, q.attemptId, q.projectId);
    ...
    if (!q.expectedRevision || a.revision !== q.expectedRevision)
      throw new ExecutionError(
        "REVISION_MISMATCH",
        "Execution revision changed; reload the attempt.",
      );
    const saved = f(a, c);
...
export function finishExecution(
  ...
  return mutateExecution(q, (a, c) => {
    if (
      ...
      !["succeeded", "failed", "stopped"].includes(q.outcome)
    )
...
export function verifyExecution(
  ...
  return mutateExecution(q, (a, c) => {
    if (
      ...
      !q.command?.trim() ||
      !q.evidence?.trim() ||
      !["passed", "failed"].includes(q.outcome)
    )
      throw new ExecutionError(
        "EXECUTION_INVALID",
        "Verification requires an ended, undecided attempt, command and durable evidence.",
      );
    const snapshot = captureSnapshot(executionRepo(c, a));
    a.snapshot = snapshot;
    a.verifications.push({
      command: q.command,
      outcome: q.outcome,
      at: new Date().toISOString(),
      snapshot,
      evidence_ref: evidence(c.root, a.id, q.evidence),
      evidence_digest: evidenceDigest(q.evidence),
    });
  });
}
  ...
  return mutateExecution(q, (a, c) => {
    ...
    if (q.decision === "accepted") {
      ...
      if (
        ...
        !v ||
        v.outcome !== "passed" ||
        !evidenceReadable(c.root, v.evidence_ref, v.evidence_digest)
      )
        throw new ExecutionError(
          "EXECUTION_CONFLICT",
          "Acceptance requires successful execution and readable passing evidence.",
        );
      ...
      if (
        ...
        throw new ExecutionError(
          "EXECUTION_CONFLICT",
          "Managed work must be committed before acceptance; submit the changes, then verify that commit.",
        );
      ...
      const item = {
        ...
        fixed_at: new Date().toISOString().slice(0, 10),
      ...
      a.acceptance = {
        ...
        note: q.note ?? "",
        at: new Date().toISOString(),
        snapshot_digest: a.snapshot?.digest ?? null,
      };
      ...
      try {
        ...
        if (itemWritten) {
          try {
            updateItemFile(c.backlog, shown.data.item);
          } catch (failure) {
            failures.push(failure);
          }
          try {
        ...
        if (failures.length)
          throw new ExecutionError(
            "EXECUTION_INVALID",
            "Acceptance write failed and rollback was incomplete; inspect the attempt and task before retrying.",
          );
        throw error;
      }
      return true;
