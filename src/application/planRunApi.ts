...
import { context, evidenceReadable, ExecutionError, listAttempts, readAttempt } from '../execution/store.js';
...
function acceptedAttempt(c: Context, attempts: ExecutionAttempt[], attemptId: string, input: PlanRunNode['input'], baseline?: CodeSnapshot): ExecutionAttempt {
  ...
  if (attempt.item_id !== input.id || latest?.id !== attempt.id || attempt.state !== 'succeeded' ||
      attempt.acceptance?.decision !== 'accepted' || !sameTaskInput(attempt.input.item, input) ||
      !attempt.snapshot || attempt.snapshot.digest !== attempt.acceptance.snapshot_digest || checks.size === 0 ||
      [...checks.values()].some(v => v.outcome !== 'passed' || !evidenceReadable(c.root, v.evidence_ref, v.evidence_digest)) ||
      (baseline !== undefined && baseline.digest !== attempt.acceptance.snapshot_digest))
    throw new ExecutionError('EXECUTION_CONFLICT', `Task ${input.id} requires a current accepted attempt, matching baseline and passing durable evidence.`);
  return attempt;
}
...
export function createPlanRun(q: PlanRunQuery & { planId: string; expectedRevision: string; reuse?: PlanRunReuse[]; instructions?: string }) {
  return executionResult(() => {
    ...
    if (new Set(Object.values(mapping)).size !== Object.values(mapping).length) throw new ExecutionError('EXECUTION_INVALID', 'Plan mapping contains duplicate task IDs.');
    const reuse = new Map((q.reuse ?? []).map(entry => [entry.itemId, entry]));
    if (reuse.size !== (q.reuse ?? []).length) throw new ExecutionError('EXECUTION_INVALID', 'Duplicate reuse task.');
    const reuseTask = (input: PlanRunNode['input']) => {
      const choice = reuse.get(input.id);
      if (input.status !== 'done' || !choice?.note.trim()) throw new ExecutionError('EXECUTION_CONFLICT', `Task ${input.id} needs explicit accepted-attempt reuse evidence and a human reuse note.`);
      acceptedAttempt(c, attempts, choice.attemptId, input, baseline);
      reuse.delete(input.id);
      return choice;
    };
    const nodes: PlanRunNode[] = plan.items.filter(item => item.item_type === 'task').map(draft => {
      const input = task(q, mapping[draft.key]!);
      if (input.item_type !== 'task' || input.source !== `plan:${plan.id}#${draft.key}`) throw new ExecutionError('EXECUTION_INVALID', 'Plan task mapping does not match its source.');
      const expected = draft.depends_on.map(key => mapping[key]!);
      if (expected.some(id => !input.depends_on.includes(id))) throw new ExecutionError('EXECUTION_INVALID', 'Materialized dependency graph is missing Plan dependencies.');
      if (!['todo', 'in_progress', 'done'].includes(input.status)) throw new ExecutionError('EXECUTION_CONFLICT', `Task ${input.id} is blocked or cancelled.`);
      const choice = input.status === 'done' ? reuseTask(input) : undefined;
      const history = attempts.filter(attempt => attempt.item_id === input.id).map(attempt => attempt.id);
      return { key: draft.key, item_id: input.id, input: structuredClone(input), depends_on: [...input.depends_on], state: choice ? 'accepted' : 'pending', attempt_ids: history, accepted_attempt_id: choice?.attemptId ?? null, reuse_note: choice?.note ?? null };
    });
    if (!nodes.length) throw new ExecutionError('EXECUTION_CONFLICT', 'Plan has no executable tasks.');
    ...
    for (const node of nodes) for (const dependency of node.depends_on) {
      ...
      if (input.item_type !== 'task') throw new ExecutionError('EXECUTION_INVALID', 'Only task dependencies can participate in execution.');
    ...
    if (reuse.size) throw new ExecutionError('EXECUTION_INVALID', 'Reuse entries must name completed tasks or dependencies in this run.');
    const reachable = new Set(externalIds);
    while (true) {
      const ready = nodes.filter(node => !reachable.has(node.item_id) && node.depends_on.every(id => reachable.has(id)));
...
export function listPlanRuns(q: PlanRunQuery & { planId?: string }) {
  return executionResult(() => {
    const c = context(q.workspaceDir, q.projectId);
    return { runs: listStoredPlanRuns(planRunRoot(c.root), q.projectId).filter(run => !q.planId || run.plan_id === q.planId) };
  });
}
...
export function validatePlanRunCompletion(q: PlanRunDetailQuery) {
  return executionResult(() => {
    ...
    const latest = listStoredPlanRuns(root, q.projectId).find(entry => entry.plan_id === run.plan_id);
    ...
    if (history.some(attempt => activeStates.includes(attempt.state)))
      throw new ExecutionError('EXECUTION_CONFLICT', 'Active or unknown work prevents completed delivery.');
    const attempts = run.nodes.map(node => {
      if (node.state !== 'accepted' || !node.accepted_attempt_id || node.accepted_attempt_id !== node.attempt_ids.at(-1) ||
          (node.input.status === 'done' && !node.reuse_note?.trim()))
        throw new ExecutionError('EXECUTION_CONFLICT', `Task ${node.item_id} lacks accepted completion or explicit reuse provenance.`);
      return acceptedAttempt(c, history, node.accepted_attempt_id, node.input);
    });
    for (const dependency of run.external_dependencies) {
      if (!dependency.note.trim()) throw new ExecutionError('EXECUTION_CONFLICT', 'Dependency reuse needs an inspection note.');
...
function inputsValid(q: PlanRunQuery, c: Context, run: PlanRun): void {
  ...
  for (const node of run.nodes) {
    ...
    if (node.state === 'accepted' && current.status !== 'done') throw new ExecutionError('EXECUTION_CONFLICT', `Accepted task status changed: ${node.item_id}.`);
    if (['blocked', 'cancelled'].includes(current.status)) throw new ExecutionError('EXECUTION_CONFLICT', `Task ${node.item_id} is ${current.status}.`);
  }
  for (const entry of run.external_dependencies) {
    const current = task(q, entry.input.id);
    if (current.status !== 'done' || !sameTaskInput(current, entry.input)) throw new ExecutionError('EXECUTION_CONFLICT', `Dependency input changed: ${entry.input.id}.`);
  }
}
...
export class PlanRunRuntime {
  ...
  private mutate(q: PlanRunMutation, operation: (run: PlanRun, c: Context) => void): ApplicationResult<PlanRunDetail> {
    return executionResult(() => {
      const c = context(q.workspaceDir, q.projectId);
      const root = planRunRoot(c.root);
      const run = readPlanRun(root, q.runId, q.projectId);
      if (!q.expectedRevision || q.expectedRevision !== run.revision) throw new ExecutionError('REVISION_MISMATCH', 'Plan run revision changed; reload before controlling it.');
  ...
  private observe(run: PlanRun, c: Context): void {
    ...
    for (const dependency of run.external_dependencies) {
      try { acceptedAttempt(c, attempts, dependency.attempt_id, dependency.input); }
      catch (error) { problems.push(error instanceof ExecutionError ? error.message : 'Dependency evidence is unavailable.'); }
    }
    for (const node of run.nodes) {
      try {
        ...
        if (attempt.item_id !== node.item_id) throw new ExecutionError('EXECUTION_INVALID', 'Run attempt belongs to another task.');
        if (node.state === 'accepted') {
          acceptedAttempt(c, attempts, attemptId, node.input);
          continue;
        }
        if (node.state === 'pending' && !activeStates.includes(attempt.state) && attempt.acceptance?.decision !== 'accepted') continue;
        if (attempt.acceptance?.decision === 'accepted') {
          ...
          node.state = 'unknown';
          throw new ExecutionError('EXECUTION_CONFLICT', `Attempt ${attemptId} is unknown; confirm prior work stopped before resuming.`);
          ...
          node.state = 'failed';
          throw new ExecutionError('EXECUTION_CONFLICT', `Task ${node.item_id} ended without acceptance; inspect and explicitly resume.`);
        }
      } catch (error) { problems.push(error instanceof ExecutionError ? error.message : `Task ${node.item_id} execution is unavailable.`); }
    }
  ...
  advance(q: PlanRunMutation) {
    return this.mutate(q, (run, c) => {
      ...
      for (const inspect of [() => this.observe(run, c), () => inputsValid(q, c, run)]) {
        try { inspect(); }
        catch (error) { problems.push(error instanceof ExecutionError ? error.message : 'Plan run inputs or execution records are unavailable.'); }
      }
      if (problems.length) { run.state = 'paused'; run.diagnostics = problems; return; }
      try {
        if (paused) return;
        if (run.nodes.every(node => node.state === 'accepted')) { run.state = 'completed'; run.diagnostics = []; return; }
        if (run.nodes.some(node => ['running', 'awaiting_acceptance'].includes(node.state))) { run.state = 'running'; return; }
        if (captureSnapshot(c.repo).digest !== run.baseline.digest) throw new ExecutionError('EXECUTION_CONFLICT', 'Repository baseline changed; inspect it before resuming.');
        const node = nextReadyNode(run);
        if (!node) throw new ExecutionError('EXECUTION_CONFLICT', 'No task is ready; dependencies require accepted evidence.');
        const history = listAttempts(c.root, q.projectId).filter(attempt => attempt.item_id === node.item_id);
        ...
        run.state = 'paused';
        run.diagnostics = [error instanceof ExecutionError ? error.message : 'Plan run could not inspect its current inputs or execution records.'];
      }
    });
  }
  ...
  pause(q: PlanRunMutation & { note?: string }) {
    return this.mutate(q, run => {
      if (['completed', 'stopped'].includes(run.state)) throw new ExecutionError('EXECUTION_CONFLICT', 'Ended run cannot be paused.');
      run.state = 'paused';
      run.controls.push({ action: 'pause', note: q.note ?? 'Paused by user.', at: new Date().toISOString(), baseline_digest: run.baseline.digest });
    });
  }

  resume(q: PlanRunMutation & { note: string; baselineDigest?: string }) {
    return this.mutate(q, (run, c) => {
      if (run.state !== 'paused' || !q.note?.trim()) throw new ExecutionError('EXECUTION_CONFLICT', 'Resume requires a paused run and an inspection note.');
      inputsValid(q, c, run);
      const attempts = listAttempts(c.root, q.projectId);
      if (attempts.some(attempt => activeStates.includes(attempt.state))) throw new ExecutionError('EXECUTION_CONFLICT', 'Running, stopping or unknown work must settle before resume.');
      const baseline = captureSnapshot(c.repo);
      for (const dependency of run.external_dependencies) acceptedAttempt(c, attempts, dependency.attempt_id, dependency.input);
      for (const node of run.nodes) {
        ...
        if (latest.acceptance?.decision === 'accepted') {
          acceptedAttempt(c, attempts, latestId, node.input, node.state === 'accepted' ? undefined : baseline);
          node.state = 'accepted'; node.accepted_attempt_id = latestId;
        } else node.state = latest.state === 'succeeded' && !latest.acceptance ? 'awaiting_acceptance' : 'pending';
      }
      if (baseline.digest !== run.baseline.digest && q.baselineDigest !== baseline.digest)
        throw new ExecutionError('EXECUTION_CONFLICT', 'Repository baseline changed; supply its current baselineDigest after inspection.');
      if (q.baselineDigest !== undefined && q.baselineDigest !== baseline.digest) throw new ExecutionError('REVISION_MISMATCH', 'Inspected repository baseline changed.');
      run.baseline = baseline;
      run.state = run.nodes.every(node => node.state === 'accepted') ? 'completed' : 'ready'; run.diagnostics = [];
      run.controls.push({ action: 'resume', note: q.note, at: new Date().toISOString(), baseline_digest: baseline.digest });
    });
  }

  stopCurrent(q: PlanRunMutation & { note?: string }) {
    return this.mutate(q, (run, c) => {
      const active = run.nodes.flatMap(node => node.attempt_ids.at(-1) ? [readAttempt(c.root, node.attempt_ids.at(-1)!, q.projectId)] : []).find(attempt => attempt.state === 'running');
      if (!active) throw new ExecutionError('EXECUTION_CONFLICT', 'Plan run has no running attempt to stop.');
      unwrap(this.execution.stop({ ...q, attemptId: active.id, expectedRevision: active.revision }));
      run.state = 'paused';
      run.controls.push({ action: 'stop', note: q.note ?? 'Stop current task requested.', at: new Date().toISOString(), baseline_digest: run.baseline.digest });
    });
  }
  ...
  closeStopped(q: PlanRunMutation & { note: string }) {
    return this.mutate(q, (run, c) => {
      if (['completed', 'stopped'].includes(run.state) || !q.note?.trim())
        throw new ExecutionError('EXECUTION_CONFLICT', 'Ending a Plan scope requires an unfinished run and an inspection note.');
      if (listAttempts(c.root, q.projectId).some(attempt => activeStates.includes(attempt.state)))
        throw new ExecutionError('EXECUTION_CONFLICT', 'Active or unknown work must be confirmed stopped before ending the Plan scope.');
      run.state = 'stopped';
      run.controls.push({ action: 'close_stopped', note: q.note, at: new Date().toISOString(), baseline_digest: run.baseline.digest });
    });
  }
}
