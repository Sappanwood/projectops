...
export function getExecutionEvidence(q: AttemptQuery & { ref: string }) {
  return executionResult(() => {
    ...
    if (q.itemId && attempt.item_id !== q.itemId)
    ...
    try {
      ...
      if (!isWithinWorkspace(realpathSync(c.root), file) || !statSync(file).isFile())
        throw new Error("Invalid evidence target.");
      const body = readFileSync(file, "utf8");
      if (!body.trim() || evidenceDigest(body) !== verification.evidence_digest)
        throw new Error("Invalid evidence digest.");
      return { ref: q.ref, body: body.slice(0, 65536), truncated: body.length > 65536 };
    } catch {
      throw new ExecutionError(
        "EXECUTION_INVALID",
        "Evidence is unavailable or its content has changed.",
      );
    }
  });
}
