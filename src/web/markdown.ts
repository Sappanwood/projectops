  ...
  for (const match of text.matchAll(tokens)) {
    ...
    if (match[1] !== undefined) html += `<code>${e(match[1])}</code>`;
      ...
      html += /^https?:\/\//i.test(url)
        ...
        : internal?.startsWith("#/projects/")
          ? `<a href="${e(internal)}">${e(match[2])}</a>`
  ...
  function list(indent: number): string {
    ...
    while (i < lines.length) {
      ...
      while (i < lines.length) {
        ...
        else if (!next && lines[i]!.trim() && lines[i]!.search(/\S/) > indent)
          html += ` ${inlineText(lines[i++]!.trim())}`;
        else break;
      }
      html += "</li>";
    }
  ...
  while (i < lines.length) {
    ...
    if (isTable(i)) {
      ...
      while (i < lines.length && lines[i]!.trim() && lines[i]!.includes("|")) {
        ...
        rows.push(
          `<tr>${headers.map((_, column) => `<td>${inlineText(row[column] ?? "")}</td>`).join("")}</tr>`,
        );
      }
      output.push(
        `<div class="table-scroll" tabindex="0" role="region" aria-label="表格"><table><thead><tr>${headers.map((cell) => `<th>${inlineText(cell)}</th>`).join("")}</tr></thead><tbody>${rows.join("")}</tbody></table></div>`,
      );
      continue;
    }
    if (/^\s*>/.test(line)) {
    ...
    if (heading) {
      ...
      output.push(
        `<h${level}${id ? ` id="${e(id)}" tabindex="-1"` : ""}>${inlineText(heading[2]!)}</h${level}>`,
      );
      i++;
      continue;
    }
    ...
    if (bullet) {
      output.push(list(bullet[1]!.length));
      continue;
    }
    ...
    while (
      i < lines.length &&
      lines[i]!.trim() &&
      !/^(#{1,6}\s|\s*`{3}|\s*~{3})/.test(lines[i]!) &&
      !listMatch(lines[i]!) &&
      !isTable(i) &&
      !/^\s*>|^\s{0,3}([-*_])(?:\s*\1){2,}\s*$/.test(lines[i]!)
    )
...
export function renderReadingBody(
  body: string,
  key: string,
