...
export type ApiClient = {
  ...
  updateBacklog(projectId: string, itemId: string, status: string, revision: string): Promise<ApiResult<BacklogMutationReceipt>>;
};
...
export function createApiClient(
  ...
  return {
    ...
    getProjectOverview(projectId: string) {
      return request<WorkbenchProjectOverview>(
    ...
    showBacklog(projectId, itemId) {
      return request(`/api/projects/${encodeURIComponent(projectId)}/backlog/${encodeURIComponent(itemId)}`);
    },
