...
type Panel = { runs: ParallelRun[]; selected: string; busy: boolean; message: string; note: string; head: string; rework: Record<string, string>; request: number };
const labels: Record<string, string> = { ready: '待启动', running: '运行中', paused: '已暂停', completed: '全部落地完成', stopped: '并行执行已终止', pending: '等待依赖与资源', awaiting_acceptance: '等待人工验收', awaiting_landing: '已验收，等待落地', landing: '落地候选处理中；异常时需人工核对', landed: '已落地', failed: '失败，需人工恢复', unknown: '状态未知，需核对旧工作' };
const button = (action: string, label: string, disabled = false) => `<button type="button" class="btn btn-secondary" data-parallel-action="${action}" ${disabled ? 'disabled' : ''}>${label}</button>`;
export function createParallelRunUi(container: HTMLElement, api: ApiClient, getState: () => AppState) {
  ...
  function panel(project: string, plan: string): Panel {
    ...
    if (!value) { value = { runs: [], selected: '', busy: false, message: '', note: '', head: '', rework: {}, request: 0 }; panels.set(id, value); void load(project, plan, value); }
    return value;
  }
  async function load(project: string, plan: string, value: Panel) {
    if (!api.request || value.busy) return;
    const request = ++value.request;
    const result = await api.request<{ runs: ParallelRun[] }>(`${base(project)}?plan_id=${encodeURIComponent(plan)}`);
    if (destroyed || request !== value.request) return;
    if (result.ok) { value.runs = result.data.runs.toSorted((a, b) => b.created_at.localeCompare(a.created_at)); if (!value.selected) value.selected = value.runs[0]?.id ?? ''; }
  ...
  function render() {
    ...
    for (const card of container.querySelectorAll<HTMLElement>('[data-plan-id]')) {
      ...
      const permission = state.readPages?.plans.find(entry => entry.id === plan)?.execution_policy?.max_parallel === 2;
      const value = panel(project, plan);
      let slot = card.querySelector<HTMLElement>('[data-parallel-panel]');
      if (!slot) { slot = container.ownerDocument.createElement('section'); slot.dataset.parallelPanel = plan; slot.setAttribute('aria-label', `并行执行 ${plan}`); card.append(slot); }
      ...
      const field = focused?.dataset.parallelField, reworkField = focused?.dataset.parallelReworkNote, selection = focused ? [focused.selectionStart, focused.selectionEnd] : null;
      ...
      slot.innerHTML = `<h3>有界并行执行</h3><p>${permission ? '计划已显式允许并行，最大容量为 2。' : '计划尚未显式允许并行；请先修订并批准并行许可。'} 每个任务使用独立 checkout，前置任务验收并落地后才解锁依赖。</p>${button('refresh', '刷新并行执行', value.busy)} ${button('create', '创建有界并行执行', value.busy || !permission || state.readPages?.plans.find(entry => entry.id === plan)?.status === 'done')}
        ...
        ${run ? `<article><h4>${e(run.plan_snapshot.title)} · ${e(labels[run.state] ?? run.state)}</h4><p>执行 ID：${e(run.id)} · 冻结计划 revision：${e(run.plan_revision)} · 并发容量：${run.capacity}</p>
          ...
          <ol>${run.nodes.map(node => `<li data-parallel-node="${e(node.key)}"><a href="#/projects/${encodeURIComponent(project)}/backlog/${encodeURIComponent(node.item_id)}?plan=${encodeURIComponent(plan)}">${e(node.item_id)} — ${e(node.input.title)}</a><p>${e(labels[node.state] ?? node.state)} · ${node.parallel ? '允许并行' : '独占运行'} · 资源：${e(node.resources.join(', ') || '无声明资源')} · 依赖：${e(node.depends_on.join(', ') || '无')}</p><p>尝试：${e(node.attempt_ids.join(', ') || '尚未启动')}</p><p>任务 checkout：<code>${e(node.workspace?.dir ?? '尚未创建')}</code></p>
            ...
            ${!terminal && node.state === 'awaiting_landing' ? `<button type="button" class="btn btn-secondary" data-parallel-land="${e(node.key)}" ${value.busy ? 'disabled' : ''}>验证并落地 ${e(node.input.title)}</button>` : ''}</li>`).join('')}</ol>
          ${run.diagnostics.map(message => `<p role="alert">${e(message)}</p>`).join('')}
          ${run.state === 'ready' ? button('advance', '启动并行任务', value.busy) : ''}
          ${!terminal ? `${button('pause', '暂停并行派发', value.busy || run.state === 'paused')}<p>暂停后已运行的任务仍可结束。请进入任务页检查、停止或验收单个任务。</p>
      ...
      for (const detail of slot.querySelectorAll<HTMLDetailsElement>('details')) if (opened.has(detail.dataset.parallelDetails)) detail.open = true;
      if (field || reworkField) { const target = field ? slot.querySelector<HTMLTextAreaElement>(`[data-parallel-field="${field}"]`) : slot.querySelector<HTMLTextAreaElement>(`[data-parallel-rework-note="${reworkField}"]`); target?.focus({ preventScroll: true }); if (target && selection) target.setSelectionRange(selection[0]!, selection[1]!); }
    }
  }
  async function act(target: HTMLElement) {
    if (!api.request) return;
    const plan = target.closest<HTMLElement>('[data-parallel-panel]')?.dataset.parallelPanel, project = getState().selectedProjectId;
    if (!plan || !project) return;
    const value = panel(project, plan);
    if (value.busy) return;
    if (target.dataset.parallelId) { value.selected = target.dataset.parallelId; render(); return; }
    const action = target.dataset.parallelRework ? 'rework' : target.dataset.parallelLand ? 'land' : target.dataset.parallelAction!;
    ...
    if (action === 'create') {
      ...
      result = await api.request<{ run: ParallelRun; diagnostics: string[] }>(`${base(project)}/${encodeURIComponent(run.id)}/${action}`, { expected_revision: run.revision,
        ...(action === 'resume' ? { note: value.note, ...(value.head.trim() ? { integration_head: value.head.trim() } : {}) } : action === 'close-stopped' ? { note: value.note } : action === 'rework' ? { node_key: target.dataset.parallelRework, note: value.rework[target.dataset.parallelRework!] } : action === 'land' ? { node_key: target.dataset.parallelLand } : {}) }, 'POST');
    }
    if (destroyed) return;
    value.busy = false;
  ...
  function onClick(event: Event) { const target = (event.target as HTMLElement)?.closest<HTMLElement>('[data-parallel-action], [data-parallel-id], [data-parallel-land], [data-parallel-rework]'); if (target) { event.preventDefault(); void act(target); } }
  function onInput(event: Event) {
    const target = event.target as HTMLInputElement;
    const plan = target.closest<HTMLElement>('[data-parallel-panel]')?.dataset.parallelPanel, project = getState().selectedProjectId;
    const field = target.dataset.parallelField;
    if (plan && project && target.dataset.parallelReworkNote) {
      const value = panel(project, plan), node = target.dataset.parallelReworkNote; value.rework[node] = target.value;
      const submit = target.closest('[data-parallel-node]')?.querySelector<HTMLButtonElement>('[data-parallel-rework]'); if (submit) submit.disabled = value.busy || !target.value.trim();
    }
    if (plan && project && (field === 'note' || field === 'head')) {
      const value = panel(project, plan); value[field] = target.value;
      const resume = target.closest('[data-parallel-panel]')?.querySelector<HTMLButtonElement>('[data-parallel-action="resume"]'); if (resume) resume.disabled = value.busy || !value.note.trim();
      const close = target.closest('[data-parallel-panel]')?.querySelector<HTMLButtonElement>('[data-parallel-action="close-stopped"]');
      const run = value.runs.find(entry => entry.id === value.selected); if (close) close.disabled = value.busy || !value.note.trim() || !!run?.nodes.some(node => ['running', 'unknown', 'landing'].includes(node.state));
    }
  }
