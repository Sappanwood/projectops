...
export type ModelSelectionState = ModelCatalog & { selected: ModelRef | null; loading: boolean; error: string };
...
export function rememberModel(selected: ModelRef | null): void {
  try { globalThis.localStorage?.setItem(storageKey, JSON.stringify(selected)); } catch { /* Keep the session selection. */ }
}
export function renderModelSelector(state: ModelSelectionState): string {
  ...
  const found = !state.selected || state.models.some(m => m.provider === state.selected!.provider && m.id === state.selected!.id);
  const providers = [...new Set(state.models.map(m => m.provider))].sort();
  const message = state.error || (state.loading ? '正在读取模型…' : !state.available ? '服务尚未启用 Pi' : !state.models.length ? '请在本地 Pi 完成认证后刷新' : !found ? '所选模型不可用，请刷新或重新选择' : '仅用于新任务；计划运行在创建时固定模型');
  return `<div class="model-selector"><label for="model-select">模型</label>
    <select id="model-select" aria-describedby="model-selection-status" ${state.loading || !state.available ? 'disabled' : ''}>
      ...
      ${providers.map(provider => `<optgroup label="${e(provider)}">${state.models.filter(m => m.provider === provider).map(m => {
