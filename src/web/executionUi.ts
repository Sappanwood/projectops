...
const active = (attempt: ExecutionAttempt) => ['running','stop_requested','unknown'].includes(attempt.state);
const labels: Record<string,string> = { running:'运行中', stop_requested:'已请求停止，等待确认', unknown:'运行状态待确认', succeeded:'执行成功（尚需验收）', failed:'执行失败', stopped:'已停止' };
const button = (action:string,label:string,disabled=false) => `<button type="button" class="btn btn-secondary" data-execution-action="${action}" ${disabled?'disabled':''}>${label}</button>`;
export function createExecutionUi(container: HTMLElement, api: ApiClient, getState:()=>AppState, updated:()=>Promise<void>) {
  let key=''; let project=''; let item=''; let attempts:ExecutionAttempt[]=[]; let selected:AttemptDetail|null=null;
  let busy=false; let message=''; let loading=false; let generation=0; let requestId=0; let destroyed=false;
  let note=''; let instructions=''; let steering='';
  let evidenceView: {attemptId:string;ref:string;body:string;truncated:boolean}|null=null; let runnerAvailable=false;
  const path=()=>`/api/projects/${encodeURIComponent(project)}/executions`;
  function currentKey() { const state=getState(); return state.currentView==='backlog' && state.backlog.item && !state.backlog.detailLoading ? `${state.selectedProjectId}/${state.backlog.item.id}` : ''; }
  function draw() {
    if (destroyed || !key || key!==currentKey()) return;
    const panel=container.querySelector('[aria-label="Backlog item detail"]'); if (!panel) return;
    let slot=panel.querySelector<HTMLElement>('[data-execution-panel]');
    if (!slot) { slot=container.ownerDocument.createElement('section'); slot.dataset.executionPanel=''; slot.setAttribute('aria-label','任务执行'); panel.append(slot); }
    const attempt=selected?.attempt;
    const last=attempt?.verifications.at(-1);
    const canAccept=attempt?.state==='succeeded' && !attempt.acceptance && last?.outcome==='passed' && selected?.diagnostics.length===0;
    ...
    const focusField=focused?.dataset.executionField; const selection=focused ? [focused.selectionStart,focused.selectionEnd] : null;
    const opened=new Set([...slot.querySelectorAll<HTMLDetailsElement>('details[open]')].map(d=>d.dataset.executionDetails));
    ...
    for(const detail of slot.querySelectorAll<HTMLDetailsElement>('details')) if(opened.has(detail.dataset.executionDetails))detail.open=true;
    if(focusField){const field=slot.querySelector<HTMLTextAreaElement>(`[data-execution-field="${focusField}"]`);field?.focus({preventScroll:true});if(field&&selection)field.setSelectionRange(selection[0]!,selection[1]!);}
  }
  async function load(id?:string) {
    if (!api.request || !key || busy) return;
    const run=generation; const request=++requestId; const base=path(); const targetItem=item;
    loading=true;
    const result=await api.request<{attempts:ExecutionAttempt[];runner_available:boolean}>(`${base}?item_id=${encodeURIComponent(targetItem)}`);
    if (destroyed||run!==generation||request!==requestId) return;
    loading=false;
    if (!result.ok) { message=`${result.error.code}: ${result.error.message}`; draw(); return; }
    attempts=result.data.attempts; runnerAvailable=result.data.runner_available;
    ...
    if (chosen) {
      ...
      if (destroyed||run!==generation||request!==requestId) return;
      if (detail.ok) selected=detail.data; else message=`${detail.error.code}: ${detail.error.message}`;
    } else selected=null;
    draw();
  }
  function render() {
    ...
    if(next!==key) { generation++;requestId++; key=next;attempts=[];selected=null;message='';busy=false;note='';instructions='';steering='';evidenceView=null;loading=!!next;
      if(next) { project=getState().selectedProjectId!;item=getState().backlog.item!.id;void load(); }
    }
    draw();
  }
  async function act(action:string) {
    if(!api.request||busy||!key) return;
    if(action==='refresh') { message='';await load();return; }
    const a=selected?.attempt; const run=generation; const base=path();
    ...
    busy=false;
    if(result.ok) { if(action==='steer')steering=''; selected=result.data;message=action==='accept'?'已验收，任务完成。':action==='rework'?'已记录继续修改要求。':'操作已记录。'; }
  ...
  async function readEvidence(ref:string) {
    ...
    const run=generation;const attemptId=selected.attempt.id;
    const result=await api.request<{ref:string;body:string;truncated:boolean}>(`${path()}/${encodeURIComponent(attemptId)}/evidence?ref=${encodeURIComponent(ref)}`);
    if(destroyed||run!==generation||selected?.attempt.id!==attemptId)return;
    if(result.ok){evidenceView={attemptId,...result.data};message='';}
    else {evidenceView=null;message=`证据不可读：${result.error.code}: ${result.error.message}`;}
    draw();
  }
  function onClick(event:Event) {
    const target=(event.target as HTMLElement)?.closest<HTMLElement>('[data-execution-action], [data-execution-id], [data-execution-evidence]');
    if(!target)return;event.preventDefault();
    if(target.dataset.executionEvidence) {void readEvidence(target.dataset.executionEvidence);}
    else if(target.dataset.executionId) { note='';void load(target.dataset.executionId); }
    else void act(target.dataset.executionAction!);
  }
  function onInput(event:Event) { const target=event.target as HTMLTextAreaElement; if(target.dataset.executionField==='note'){note=target.value;const confirm=container.querySelector<HTMLButtonElement>('[data-execution-action="confirm-interrupted"]');if(confirm)confirm.disabled=busy||!note.trim();} if(target.dataset.executionField==='instructions')instructions=target.value; if(target.dataset.executionField==='steering'){steering=target.value;const send=container.querySelector<HTMLButtonElement>('[data-execution-action="steer"]');if(send)send.disabled=busy||!steering.trim();} }
  container.addEventListener('click',onClick);container.addEventListener('input',onInput);
  const timer=api.request && typeof window!=='undefined' ? setInterval(()=>{if(key&&!busy&&attempts.some(active))void load();},3000) : undefined;
  return {render,destroy(){destroyed=true;generation++;clearInterval(timer);container.removeEventListener('click',onClick);container.removeEventListener('input',onInput);}};
}
