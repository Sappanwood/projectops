...
const labels: Record<string, string> = { todo: "待办", in_progress: "进行中", blocked: "受阻", done: "已完成", cancelled: "已取消" };

export function renderBacklogPanel(state: BacklogViewState): string {
  if (state.loading) return '<p role="status">Loading backlog…</p>';
  if (state.error) return `<p role="alert">${e(state.error.message)}</p><button class="btn btn-secondary" id="backlog-refresh">Retry backlog</button>`;
  const groups = ["todo", "in_progress", "blocked", "done", "cancelled"].map((status) => {
    ...
    return `<section class="backlog-group"><h3>${labels[status]} <span class="group-count">${items.length}</span></h3><ul class="items-list">${items.map((item) =>
      `<li><button class="backlog-select" data-backlog-item="${e(item.id)}" ${state.saving ? "disabled" : ""}
        ...
        <span class="backlog-select-title">${e(item.title)}</span><span class="item-meta"><code>${e(item.id)}</code><span class="badge badge-priority">${e(item.priority)}</span><span class="badge badge-${e(status)}">${labels[status]}</span></span></button></li>`).join("")}</ul></section>`;
  }).join("");
  ...
  if (state.detailError) detail += `<p class="reading-notice" role="alert">${e(state.detailError.code ?? "ERROR")}: ${e(state.detailError.message)}</p>`;
  if (item && !state.detailLoading) {
    detail += `<header class="reading-header"><div class="item-meta"><code>${e(item.id)}</code><span class="badge badge-priority">${e(item.priority)}</span><span class="badge badge-${e(item.status)}" aria-label="Status: ${e(item.status)}">${labels[item.status] ?? e(item.status)}</span></div>
      <h3>${e(item.title)}</h3>
      <div class="reading-actions"><div class="status-actions" aria-label="Update item status"><span class="muted">状态</span>${["todo", "in_progress", "done"].map((status) =>
        `<button class="btn ${item.status === status ? "btn-primary" : "btn-secondary"}" aria-label="${status}" aria-pressed="${item.status === status}" data-backlog-status="${status}" ${state.saving || state.detailError?.code === "REVISION_MISMATCH" ? "disabled" : ""}>${labels[status]}</button>`).join(" ")}</div>
  ...
  } else if (state.selectedItemId !== null) detail += `<button class="btn btn-secondary" id="backlog-item-refresh" ${state.saving ? "disabled" : ""}>Refresh item</button>`;
