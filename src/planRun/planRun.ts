...
export type PlanRunNodeState = 'pending' | 'running' | 'awaiting_acceptance' | 'accepted' | 'failed' | 'unknown';
...
export type PlanRun = {
  ...
  controls: Array<{ action: 'pause' | 'resume' | 'stop' | 'close_stopped'; note: string; at: string; baseline_digest: string }>;
  diagnostics: string[];
};
...
export function planRunRevision(run: PlanRun): string {
  return createHash('sha256').update(JSON.stringify({ ...run, revision: '' })).digest('hex').slice(0, 16);
}

export function nextReadyNode(run: PlanRun): PlanRunNode | undefined {
  if (run.nodes.some(node => ['running', 'unknown', 'awaiting_acceptance'].includes(node.state))) return undefined;
  const accepted = new Set([
    ...run.nodes.filter(node => node.state === 'accepted').map(node => node.item_id),
    ...run.external_dependencies.map(entry => entry.input.id),
  ]);
  return run.nodes.filter(node => node.state === 'pending' && node.depends_on.every(id => accepted.has(id)))
    .sort((a, b) => a.input.priority.localeCompare(b.input.priority) || a.item_id.localeCompare(b.item_id))[0];
}
