...
export class LandingUncertainError extends Error {
  ...
  constructor(
    ...
    super(
      `Integration ref advanced to ${confirmation.candidateCommit} from ${confirmation.baseCommit}, but confirmation failed. Inspect ${confirmation.candidateDir} and ${confirmation.verificationFile}; do not retry landing before inspection.`,
      { cause },
    );
  }
}
...
function identifier(value: string) {
  if (!/^[a-zA-Z0-9][a-zA-Z0-9_-]{0,80}$/.test(value))
    throw Error("Invalid run or node identifier.");
  return value;
}
...
function within(root: string, target: string) {
  ...
  if (relative === ".." || relative.startsWith(`..${path.sep}`) || path.isAbsolute(relative))
    throw Error("Path is outside declared workspace.");
}
function gitMetadata(workspace: string, repo: string, expectedCommon?: string) {
  if (lstatSync(path.join(repo, ".git")).isSymbolicLink())
    throw Error("Git metadata contains a symbolic link.");
  ...
  within(workspace, gitDir);
  within(workspace, common);
  if (expectedCommon !== undefined && common !== expectedCommon)
    throw Error("Worktree Git metadata does not belong to its repository.");
  return common;
}
function integrationAt(run: RunWorkspace, expectedHead: string) {
  ...
  if (
    git(run.integrationDir, "rev-parse", "--symbolic-full-name", "HEAD") !== "HEAD" ||
...
function readOwned<T>(file: string): T {
  ...
  if (!stat.isFile() || stat.isSymbolicLink())
    throw Error("Ownership record is not a regular file.");
...
function commit(repo: string, value: string) {
  if (!/^(?:[a-f0-9]{40}|[a-f0-9]{64})$/.test(value))
    throw Error("An explicit commit object ID is required.");
  return git(repo, "rev-parse", "--verify", "--end-of-options", `${value}^{commit}`);
}
function assertRun(run: RunWorkspace) {
  identifier(run.runId);
  const workspace = directory(run.workspaceDir);
  directory(run.repo);
  within(workspace, run.repo);
  const root = path.join(workspace, ".pops", "runtime", "plan-runs", run.runId);
  if (
    run.root !== root ||
    run.integrationDir !== path.join(root, "integration") ||
    run.integrationRef !== `refs/heads/pops/${run.runId}/integration`
  )
    throw Error("Run ownership does not match its declared paths.");
  const owner = readOwned<RunWorkspace>(path.join(root, "ownership.json"));
  for (const key of Object.keys(run) as (keyof RunWorkspace)[])
    if (owner[key] !== run[key]) throw Error("Run ownership record does not match.");
...
function assertNode(run: RunWorkspace, node: NodeWorkspace) {
  ...
  if (
    ...
    node.dir !== path.join(parent, "worktree") ||
    node.ref !== `refs/heads/pops/${run.runId}/nodes/${node.nodeId}` ||
    Object.keys(owner).some(
      (key) => owner[key as keyof NodeWorkspace] !== node[key as keyof NodeWorkspace],
    )
  )
    throw Error("Node is not owned by this run.");
  directory(node.dir);
  gitMetadata(run.workspaceDir, node.dir, gitMetadata(run.workspaceDir, run.repo));
  if (git(node.dir, "symbolic-ref", "HEAD") !== node.ref)
    throw Error("Node worktree branch changed.");
}
...
export function createRunWorkspace(input: {
  workspaceDir: string;
  repo: string;
  runId: string;
  baseCommit: string;
  ...
  const workspaceDir = directory(input.workspaceDir);
  const repo = directory(input.repo);
  within(workspaceDir, repo);
  gitMetadata(workspaceDir, repo);
  if (git(repo, "rev-parse", "--show-toplevel") !== repo)
    throw Error("Repository must be the registered Git root.");
  const baseCommit = commit(repo, input.baseCommit);
  const runtime = ensureChild(ensureChild(workspaceDir, ".pops"), "runtime");
  const parent = ensureChild(runtime, "plan-runs");
  ...
  const run: RunWorkspace = {
    ...
    runId,
    baseCommit,
    integrationRef: `refs/heads/pops/${runId}/integration`,
  ...
  mkdirSync(path.join(root, "candidates"));
  return run;
}
export function createNodeWorkspace(
  ...
  const baseCommit = commit(
    run.repo,
    options.baseCommit ?? git(run.repo, "rev-parse", run.integrationRef),
  );
  const parent = path.join(run.root, "nodes", nodeId);
  directory(path.dirname(parent));
  mkdirSync(parent);
  const node: NodeWorkspace = {
    runId: run.runId,
    nodeId,
    baseCommit,
    dir: path.join(parent, "worktree"),
    ref: `refs/heads/pops/${run.runId}/nodes/${nodeId}`,
  };
...
function errorText(error: unknown) {
  ...
  return (
    [details.stdout, details.stderr, details.message].filter(Boolean).join("\n") ||
    "Operation failed."
  );
}
async function land(
  ...
  const baseCommit = git(run.repo, "rev-parse", run.integrationRef);
  const nodeCommit = git(node.dir, "rev-parse", "HEAD");
  integrationAt(run, baseCommit);
  const parent = path.join(run.root, "candidates", `${node.nodeId}-${randomUUID()}`);
  directory(path.dirname(parent));
  mkdirSync(parent);
  const candidateDir = path.join(parent, "worktree");
  addDetached(run.repo, candidateDir, baseCommit);
  ...
  const result = (outcome: LandingResult["outcome"], evidence: string): LandingResult => {
    ...
    writeNew(evidenceFile, value);
    return value;
  };
  try {
    git(candidateDir, "merge", "--no-ff", "--no-edit", nodeCommit);
  } catch (error) {
    return result("conflict", errorText(error));
  }
  candidateCommit = git(candidateDir, "rev-parse", "HEAD");
  let checked: Awaited<ReturnType<Verification>>;
  try {
    checked = await verify({ repo: run.repo, dir: candidateDir, baseCommit, nodeCommit });
  } catch (error) {
    return result("verification_failed", errorText(error));
  }
  if (!checked.passed || !checked.evidence?.trim())
    return result("verification_failed", checked.evidence || "Verification returned no evidence.");
  ...
  writeNew(path.join(parent, "verification.json"), {
    baseCommit,
    nodeCommit,
    candidateCommit,
    ...checked,
  ...
  try {
    git(run.repo, "update-ref", "--no-deref", run.integrationRef, candidateCommit, baseCommit);
  } catch (error) {
    return result("ref_drift", `${checked.evidence}\n${errorText(error)}`);
  }
  try {
    git(run.integrationDir, "reset", "--hard", candidateCommit);
    return result("landed", checked.evidence);
  } catch (error) {
    throw new LandingUncertainError(
      {
        candidateDir,
        candidateCommit,
        baseCommit,
        nodeCommit,
        verificationFile: path.join(parent, "verification.json"),
        evidenceFile,
      },
      error,
    );
...
export function landNode(
  ...
  void current
    .finally(() => {
      if (queues.get(run.root) === current) queues.delete(run.root);
    })
    .catch(() => {});
  return current;
}
...
export function readNodeWorkspace(
  workspaceDir: string,
  repo: string,
  runId: string,
  nodeId: string,
  ...
  const run = readOwned<RunWorkspace>(
    path.join(workspace, ".pops", "runtime", "plan-runs", runId, "ownership.json"),
  );
  if (run.workspaceDir !== workspace || run.repo !== directory(repo) || run.runId !== runId)
    throw Error("Managed checkout does not belong to the registered repository.");
  assertRun(run);
  const node = readOwned<NodeWorkspace>(path.join(run.root, "nodes", nodeId, "ownership.json"));
...
export function alignIntegration(
  run: RunWorkspace,
  expectedOldHead: string,
  newHead: string,
): void {
  assertRun(run);
  const oldCommit = commit(run.repo, expectedOldHead);
  const nextCommit = commit(run.repo, newHead);
  integrationAt(run, oldCommit);
  if (!clean(run.integrationDir))
    throw Error("Integration alignment requires a clean checkout without local changes.");
  if (git(run.repo, "rev-parse", run.integrationRef) !== nextCommit)
    throw Error("Integration ref changed after inspection.");
