...
export function planComplete(
  ...
  try {
    expectedRevision = parseArgs({
      args,
      options: { "expected-revision": { type: "string" } },
      strict: true,
      allowPositionals: false,
    }).values["expected-revision"];
  } catch {
    /* Invalid arguments use the same CLI error envelope. */
  }
  const result =
    projectId && planId && expectedRevision
      ? completePlan({ workspaceDir: cwd, projectId, planId, expectedRevision })
      : applicationFailure(
          "PLAN_INVALID",
          "Usage: pops plan complete <project> <plan> --expected-revision <revision> [--json]",
        );
