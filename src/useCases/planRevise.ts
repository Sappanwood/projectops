...
export function planRevise(
  ...
  if (!projectId || !planId) {
    io.stderr(
      "Usage: pops plan revise <project-id> <plan-id> --input <draft.json> --expected-revision <rev> [--confirm <preview-token>] [--json]",
    );
    return 1;
  }
  let values;
  ...
  try {
    ...
    io.stderr("Error: provide valid --input JSON and --expected-revision.");
    return 1;
  }
