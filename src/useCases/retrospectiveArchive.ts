...
import { archiveRetrospective, type RetrospectiveArchiveOptions } from "../retrospective/retrospectiveFs.js";
...
export function retrospectiveArchive(args: string[], json: boolean, io: CliIO, cwd: string): number {
  ...
  if (reference === undefined || values.positionals?.length !== 1) {
    return retrospectiveFailure(io, json, "Usage: pops retrospective archive <id> --expected-revision <revision> --action-disposition <value> --resolution-note <note> [--actioned-at <timestamp>] [--backlog <logical-ref>]");
  }
  ...
  if (actionedAt.trim() === "") return retrospectiveFailure(io, json, "--actioned-at must be a non-empty string");
  const backlog = [...(values.backlog ?? []), ...(values["backlog-link"] ?? [])];
  if (backlog.some((value) => value.trim() === "")) return retrospectiveFailure(io, json, "backlog links must be non-empty");
  const id = normalizeReference(reference);
  if (id === null) return retrospectiveFailure(io, json, "retrospective reference must be an id or active/<id>.md");
