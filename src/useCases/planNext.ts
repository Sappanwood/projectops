      ...
      for (const item of items) {
        ...
        if ("reasons" in item)
          for (const reason of item.reasons)
            lines.push(`  ${reason.id}: ${reason.code} — ${reason.message}`);
      }
    }
    for (const diagnostic of data.diagnostics)
      lines.push(`诊断 ${diagnostic.id}: ${diagnostic.code} — ${diagnostic.message}`);
